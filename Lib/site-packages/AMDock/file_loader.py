...
class Loader(QtGui.QMainWindow):
    ...
    def copy_target(self):
        try:
            shutil.copy('%s' % self.target.path, self.t_input)
            self.target.input = self.t_input
        except:
    ...
    def copy_offtarget(self):
        try:
            shutil.copy('%s' % self.offtarget.path, self.o_input)
            self.offtarget.input = self.o_input
        except:
            nowdir = QtGui.QMessageBox.critical(self.parent, 'Error', 'The working directory was not found or '
                                                              'it does not have permission for writing.'
                                                              '\nPlease reset the program.',
    ...
    def copy_ligand(self):
        try:
            shutil.copy('%s' % self.ligand.path, self.l_input)
            self.ligand.input = self.l_input
        except:
            nowdir = QtGui.QMessageBox.critical(self.parent, 'Error', 'The working directory was not found or '
                                                              'it does not have permission for writing.'
                                                              '\nPlease reset the program.',
    ...
    def load_protein(self):
        ...
        if data_file.exec_():
            filenames = data_file.selectedFiles()
            self.target.get_data(filenames)

            if self.target.name == self.offtarget.name:
                self.same = same_protein(self, 'Target Protein', self.target.name, 'Off-Target Protein')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.protein_text.clear()
                    self.parent.program_body.protein_label.clear()
                    self.load_protein()
            elif self.target.name == self.ligand.name:
                self.same = same_protein(self, 'Target Protein', self.target.name, 'Ligand')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.protein_text.clear()
                    self.parent.program_body.protein_label.clear()
                    self.load_protein()
            else:
                self.parent.program_body.protein_label.setText('Loaded Target Protein from: %s' % self.target.path)
                self.parent.program_body.protein_text.setText(os.path.basename(self.target.path))

                self.t_input = os.path.join(self.parent.v.input_dir, self.target.name + '.' + self.target.ext)
    ...
    def load_proteinB(self):
        ...
        if data_file.exec_():
            filenames = data_file.selectedFiles()
            self.offtarget.get_data(filenames)

            if self.offtarget.name == self.target.name:
                self.same = same_protein(self, 'Off-Target Protein', self.offtarget.name, 'Target Protein')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.protein_textB.clear()
                    self.parent.program_body.protein_labelB.clear()
                    self.load_proteinB()
            elif self.offtarget.name == self.ligand.name:
                self.same = same_protein(self, 'Off-Target Protein', self.offtarget.name, 'Ligand')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.protein_textB.clear()
                    self.parent.program_body.protein_labelB.clear()
                    self.load_proteinB()
                ...
                self.parent.program_body.protein_textB.setText(os.path.basename(self.offtarget.path))

                self.o_input = os.path.join(self.parent.v.input_dir, self.offtarget.name + '.' + self.offtarget.ext)
    ...
    def load_ligand(self):
        ...
        if data_file.exec_():
            filenames = data_file.selectedFiles()
            self.ligand.get_data(filenames)

            if self.ligand.name == self.target.name:
                self.same = same_protein(self, 'Ligand', self.ligand.name, 'Target Protein', 'ligand')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.ligand_text.clear()
                    self.parent.program_body.ligand_label.clear()
                    self.load_ligand()
            elif self.ligand.name == self.offtarget.name:
                self.same = same_protein(self, 'Ligand', self.ligand.name, 'Off-Target Protein', 'ligand')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.ligand_text.clear()
                    self.parent.program_body.ligand_label.clear()
                    self.load_ligand()
            else:
                self.parent.program_body.ligand_label.setText("Loaded Ligand from: %s" % self.ligand.path)
                self.parent.program_body.ligand_text.setText(os.path.basename(self.ligand.path))

                self.l_input = os.path.join(self.parent.v.input_dir, self.ligand.name + '.' + self.ligand.ext)
        ...
        if data_file.exec_():
            ...
            for line in self.parent.v.amdock_file:
                ...
                if re.search('>> WORKING_DIRECTORY:', line):
                    ...
                    self.ligand.name = (line.split()[2]).strip()
