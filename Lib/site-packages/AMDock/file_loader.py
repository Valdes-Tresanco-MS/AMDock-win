...
class Loader():
    ...
    def copy_target(self):
        try:
            shutil.copy('%s' % self.parent.program_body.target.path, self.t_input)
            self.parent.program_body.target.input = self.t_input
        except:
    ...
    def copy_offtarget(self):
        try:
            shutil.copy('%s' % self.parent.program_body.offtarget.path, self.o_input)
            self.parent.program_body.offtarget.input = self.o_input
        except:
            nowdir = QtGui.QMessageBox.critical(self.parent, 'Error', 'The working directory was not found or '
                                                              'it does not have permission for writing.'
                                                              '\nPlease reset the program.',
    ...
    def copy_ligand(self):
        try:
            shutil.copy('%s' % self.parent.program_body.ligand.path, self.l_input)
            self.parent.program_body.ligand.input = self.l_input
        except:
            nowdir = QtGui.QMessageBox.critical(self.parent, 'Error', 'The working directory was not found or '
                                                              'it does not have permission for writing.'
                                                              '\nPlease reset the program.',
    ...
    def load_protein(self):
        ...
        if data_file.exec_():
            filenames = data_file.selectedFiles()
            self.parent.program_body.target.get_data(filenames)

            if self.parent.program_body.target.name == self.parent.program_body.offtarget.name:
                self.same = same_protein(self, 'Target Protein', self.parent.program_body.target.name, 'Off-Target Protein')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.target_text.clear()
                    self.parent.program_body.target_label.clear()
                    self.load_protein()
            elif self.parent.program_body.target.name == self.parent.program_body.ligand.name:
                self.same = same_protein(self, 'Target Protein', self.parent.program_body.target.name, 'Ligand')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.target_text.clear()
                    self.parent.program_body.target_label.clear()
                    self.load_protein()
            else:
                self.parent.program_body.target_label.setText('Loaded Target Protein from: %s' % self.parent.program_body.target.path)
                self.parent.program_body.target_text.setText(os.path.basename(self.parent.program_body.target.path))

                self.t_input = os.path.join(self.parent.v.input_dir, self.parent.program_body.target.name + '.' + self.parent.program_body.target.ext)
                ...
                if not self.copy_target():
                    ...
                    return self.parent.program_body.target
    ...
    def load_proteinB(self):
        ...
        if data_file.exec_():
            filenames = data_file.selectedFiles()
            self.parent.program_body.offtarget.get_data(filenames)

            if self.parent.program_body.offtarget.name == self.parent.program_body.target.name:
                self.same = same_protein(self, 'Off-Target Protein', self.parent.program_body.offtarget.name, 'Target Protein')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.offtarget_text.clear()
                    self.parent.program_body.offtarget_label.clear()
                    self.load_proteinB()
            elif self.parent.program_body.offtarget.name == self.parent.program_body.ligand.name:
                self.same = same_protein(self, 'Off-Target Protein', self.parent.program_body.offtarget.name, 'Ligand')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.offtarget_text.clear()
                    self.parent.program_body.offtarget_label.clear()
                    self.load_proteinB()
            else:
                self.parent.program_body.offtarget_label.setText('Loaded Off-Target from: %s' % self.offtarget_path)
                self.parent.program_body.offtarget_text.setText(os.path.basename(self.parent.program_body.offtarget.path))

                self.o_input = os.path.join(self.parent.v.input_dir, self.parent.program_body.offtarget.name + '.' + self.parent.program_body.offtarget.ext)
        ...
        #             self.parent.program_body.wdir_button.setEnabled(False)
            # try:
            # if self.parent.v.prot_align and target_ext in ['pdb', 'ent'] and self.offtarget_ext in ['pdb', 'ent']:
    ...
    def load_ligand(self):
        ...
        if data_file.exec_():
            filenames = data_file.selectedFiles()
            self.parent.program_body.ligand.get_data(filenames)

            if self.parent.program_body.ligand.name == self.parent.program_body.target.name:
                self.same = same_protein(self, 'Ligand', self.parent.program_body.ligand.name, 'Target Protein', 'ligand')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.ligand_text.clear()
                    self.parent.program_body.ligand_label.clear()
                    self.load_ligand()
            elif self.parent.program_body.ligand.name == self.parent.program_body.offtarget.name:
                self.same = same_protein(self, 'Ligand', self.parent.program_body.ligand.name, 'Off-Target Protein', 'ligand')
                if self.same == QtGui.QMessageBox.Ok:
                    self.parent.program_body.ligand_text.clear()
                    self.parent.program_body.ligand_label.clear()
                    self.load_ligand()
            else:
                self.parent.program_body.ligand_label.setText("Loaded Ligand from: %s" % self.parent.program_body.ligand.path)
                self.parent.program_body.ligand_text.setText(os.path.basename(self.parent.program_body.ligand.path))

                self.l_input = os.path.join(self.parent.v.input_dir, self.parent.program_body.ligand.name + '.' + self.parent.program_body.ligand.ext)
                ...
                #
                # self.parent.v.input_lig = os.path.join(self.parent.v.input_dir,
                ...
                #         self.wlig = QtGui.QMessageBox.warning(self.parent, 'Warning', 'The selected ligand has more '
                #                                                                       'of 100 heavy atoms.\nDoes he '
                #                                                                       'wish to continue?',
                #                                               QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
        ...
        if data_file.exec_():
            ...
            for line in self.parent.v.amdock_file:
                ...
                if re.search('>> WORKING_DIRECTORY:', line):
                    ...
                    self.parent.program_body.ligand.name = (line.split()[2]).strip()
