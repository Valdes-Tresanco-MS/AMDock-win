...
class PDBINFO:
    ...
    def get_center(self):
        if self.exception:
            return
        minimx = min([at.coords[0] for at in self.mol.allAtoms])
        minimy = min([at.coords[1] for at in self.mol.allAtoms])
        minimz = min([at.coords[2] for at in self.mol.allAtoms])
        maximx = max([at.coords[0] for at in self.mol.allAtoms])
        maximy = max([at.coords[1] for at in self.mol.allAtoms])
        maximz = max([at.coords[2] for at in self.mol.allAtoms])
        self.center = [(minimx + maximx) / 2, (minimy + maximy) / 2, (minimz + maximz) / 2]
