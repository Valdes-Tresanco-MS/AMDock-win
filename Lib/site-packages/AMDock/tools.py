...
class PDBINFO:
    ...
    def get_box(self):
        ...
        for res in self.mol.chains.residues:
            if res.name[:3] in aa:
                self.prot.append(res)
        minimx = min([at.coords[0] for res in self.prot for at in res.atoms]) - 5
        minimy = min([at.coords[1] for res in self.prot for at in res.atoms]) - 5
        minimz = min([at.coords[2] for res in self.prot for at in res.atoms]) - 5
        maximx = max([at.coords[0] for res in self.prot for at in res.atoms]) + 5
        maximy = max([at.coords[1] for res in self.prot for at in res.atoms]) + 5
        maximz = max([at.coords[2] for res in self.prot for at in res.atoms]) + 5
        self.size = [maximx - minimx, maximy - minimy, maximz - minimz]
