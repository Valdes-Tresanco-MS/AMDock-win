...
import sys, Queue, time, traceback, re
...
class THREAD(QThread):
    ...
    def run(self):
        if self.AMDock.project.prog == 'AutoLigand':
            if re.search('Progress:', self.out):
                cursor = self.AMDock.log_widget.textedit.textCursor()
                cursor.select(QtGui.QTextCursor.LineUnderCursor)
                cursor.removeSelectedText()
        print self.AMDock.project.prog
        if not self.AMDock.project.prog in ['AutoDock Vina', 'AutoDock Vina B']:
