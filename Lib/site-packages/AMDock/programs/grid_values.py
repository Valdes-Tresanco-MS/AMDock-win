...
parser.add_argument('--f_ligands', default=None, nargs='+')  # rep_type == 3: None, rep_type == 2: lig, rep_type == 1: fill,
# rep_type == 0: fill.pdb; ...; filln.pdb
...
def load_rep(prot):
    ...
    if prot['rep_type'] == 0:
        ...
        cmd.hide("sticks",prot['prot_type']+" and rep lines")
        ...
        for res in prot['residues']:
            chain, resi = res.split(':')
            cmd.show('sticks', 'chain %s and resi %s' % (chain, resi))
            if prot['colors'][-1] == 'green':
                util.cbag('chain %s and resi %s' % (chain, resi))
            else:
                util.cbam('chain %s and resi %s' % (chain, resi))
...
if firts_prot['prot_pdb']:
    load_rep(firts_prot)
    pymol_data.write('%s %s %s %s %s %s %s' % (firts_prot['prot_type'].lower(), firts_prot['center'][0],
                                               firts_prot['center'][1], firts_prot['center'][2], firts_prot['size'][0],
                                               firts_prot['size'][1],firts_prot['size'][2]))
if sec_prot['prot_pdb']:
    load_rep(sec_prot)
    pymol_data.write('%s %s %s %s %s %s %s' % (sec_prot['prot_type'].lower(), sec_prot['center'][0],
