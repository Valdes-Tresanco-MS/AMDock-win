...
class Results(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        # self.import_help.setToolTip(self.AMDock.result_tt)
        ...
        self.load_button.clicked.connect(lambda: self.AMDock.program_body.load_file(self.load_button))
    ...
    def show_result_file(self):
        if self.AMDock.v.amdock_file is not None:
            if self.only_one == 0:
                self.rfile_show.show()
                self.only_one = 1
                self.rfile_show.lineEdit.setText(self.AMDock.v.project_name)
                self.rfile_show.lineEdit_2.setText(os.path.normpath(self.AMDock.v.WDIR))
                self.rfile_show.lineEdit_3.setText(self.AMDock.v.docking_program)
                self.rfile_show.lineEdit_4.setText(self.AMDock.v.ligand_name)
                self.rfile_show.lineEdit_5.setText(self.AMDock.v.protein_name)
                self.rfile_show.lineEdit_6.setText(str(self.AMDock.v.ligands))
                self.rfile_show.lineEdit_7.setText(str(self.AMDock.v.metals))
                self.rfile_show.lineEdit_8.setText(self.AMDock.v.result_file)
                self.rfile_show.lineEdit_9.setText(self.AMDock.v.best_result_file)
                self.rfile_show.mode_label.setText(self.AMDock.v.program_mode)
                if self.AMDock.v.analog_protein_name != 'protein B':
                    self.rfile_show.lineEdit_10.setText(str(self.AMDock.v.analog_protein_name))
                    self.rfile_show.lineEdit_11.setText(str(self.AMDock.v.analog_ligands))
                    self.rfile_show.lineEdit_12.setText(str(self.AMDock.v.analog_metals))
                    self.rfile_show.lineEdit_13.setText(str(self.AMDock.v.analog_result_file))
                    self.rfile_show.lineEdit_14.setText(str(self.AMDock.v.best_analog_result_file))
        else:
    ...
    def new_project(self, r):
        ...
        if reset_opt == QtGui.QMessageBox.Yes:
            ...
            self.AMDock.program_body.protein_text.clear()
            self.AMDock.program_body.protein_label.clear()
            self.AMDock.program_body.protein_textB.clear()
            self.AMDock.program_body.protein_labelB.clear()
            ...
            self.AMDock.program_body.progressBar.setValue(0)
            ...
            try:
                ...
                pass
            try:
                os.chdir(self.AMDock.v.loc_project)
            except:
                pass

            self.AMDock.v = Variables()
    ...
    def show_best(self, b):
        if b.objectName() == 'best_button':
            if self.AMDock.v.docking_program == 'AutoDock Vina':
                visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.result_dir,
                                                                'best_pose_target_ADV_result.pdb'),
                              self.AMDock.ws.lig_site_pymol]
            else:
                visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.result_dir,
                                                                'best_pose_target_AD4_result.pdb'),
                              self.AMDock.ws.lig_site_pymol]
            self.best_pymol = {'Pymol': [self.AMDock.ws.this_python, visual_arg]}
            self.bestw = Worker()
            self.bestw.readyReadStandardOutput.connect(self.AMDock.program_body.readStdOutput)
            self.bestw.readyReadStandardError.connect(self.AMDock.program_body.readStdError)
            self.bestw.prog_started.connect(self.AMDock.program_body.prog_show)
            self.bestw.queue_finished.connect(self.AMDock.program_body.check_queue)
            ...
            if self.AMDock.v.docking_program == 'AutoDock Vina':
                visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.result_dir,
                                                                'best_pose_off-target_ADV_result.pdb'),
                              self.AMDock.ws.lig_site_pymol]
            else:
                visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.result_dir,
                                                                'best_pose_off-target_AD4_result.pdb'),
                              self.AMDock.ws.lig_site_pymol]
            self.best_pymolB = {'Pymol': [self.AMDock.ws.this_python, visual_arg]}
            self.bestwB = Worker()
            self.bestwB.readyReadStandardOutput.connect(self.AMDock.program_body.readStdOutput)
            self.bestwB.readyReadStandardError.connect(self.AMDock.program_body.readStdError)
            self.bestwB.prog_started.connect(self.AMDock.program_body.prog_show)
            self.bestwB.queue_finished.connect(self.AMDock.program_body.check_queue)
    ...
    def show_all(self, b):
        if b.objectName() == 'all_button':
            if self.AMDock.v.docking_program == 'AutoDock Vina':
                visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.input_dir, self.AMDock.v.protein_pdbqt),
                              os.path.join(self.AMDock.v.result_dir, 'all_poses_target_ADV_result.pdb'),
                              self.AMDock.ws.protein_cartoon_pymol]
            else:
                visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.input_dir, self.AMDock.v.protein_pdbqt),
                              os.path.join(self.AMDock.v.result_dir, 'all_poses_target_AD4_result.pdb'),
                              self.AMDock.ws.protein_cartoon_pymol]
            self.all_pymol = {'Pymol': [self.AMDock.ws.this_python, visual_arg]}
            self.allw = Worker()
            self.allw.readyReadStandardOutput.connect(self.AMDock.program_body.readStdOutput)
            self.allw.readyReadStandardError.connect(self.AMDock.program_body.readStdError)
            self.allw.prog_started.connect(self.AMDock.program_body.prog_show)
            self.allw.queue_finished.connect(self.AMDock.program_body.check_queue)
            ...
            if self.AMDock.v.docking_program == 'AutoDock Vina':
                visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.input_dir,
                                                                self.AMDock.v.analog_protein_pdbqt),
                              os.path.join(self.AMDock.v.result_dir, 'all_poses_off-target_ADV_result.pdb'),
                              self.AMDock.ws.protein_cartoon_pymol]
            else:
                visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.input_dir,
                                                                self.AMDock.v.analog_protein_pdbqt),
                              os.path.join(self.AMDock.v.result_dir, 'all_poses_off-target_AD4_result.pdb'),
                              self.AMDock.ws.protein_cartoon_pymol]
            self.all_pymolB = {'Pymol': [self.AMDock.ws.this_python, visual_arg]}
            self.allwB = Worker()
            self.allwB.readyReadStandardOutput.connect(self.AMDock.program_body.readStdOutput)
            self.allwB.readyReadStandardError.connect(self.AMDock.program_body.readStdError)
            self.allwB.prog_started.connect(self.AMDock.program_body.prog_show)
            self.allwB.queue_finished.connect(self.AMDock.program_body.check_queue)
    ...
    def _show_complex(self):
        visual_arg = [self.AMDock.ws.pymol, os.path.join(self.AMDock.v.input_dir, self.AMDock.v.protein_pdbqt),
                      os.path.join(self.AMDock.v.input_dir, self.AMDock.v.ligand_pdbqt),
                      self.AMDock.ws.protein_cartoon_pymol]
        self.complex_pymol = {'Pymol': [self.AMDock.ws.this_python, visual_arg]}
