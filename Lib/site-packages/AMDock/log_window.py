...
class LogWindow(QtGui.QDockWidget):
    ...
    def file_save(self):
        if self.AMDock.project.output:
            folder = os.path.join(self.AMDock.project.WDIR, 'amdock_project.log')
        else:
            folder = os.path.join('.', 'amdock_project.log')
        name = QtGui.QFileDialog.getSaveFileName(self, 'Save Log File', folder, '*.log')
        print name
        if name:
            file = open(name, 'w')
