...
def metal_no_ADzn_od_warning(parent, text):
    ...
    if len(text) == 2:
        ...
        msg.setText("AutoDock4Zn is selected, but the following protein do not contain Zn atoms:                    "
                    "                                  \n %s (%s)" % (text[text.keys()[0]].name, text.keys()[0]))
    msg.setInformativeText('Please, make sure that both proteins contains Zn atoms. Press "OK" '
                            'if you want to select a new protein or "Cancel" if you want to change the program docking')
