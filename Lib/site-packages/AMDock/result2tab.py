...
class Result_Analysis():
    def __init__(self, prog=None, project=None, mol=None, protein=None, ha=0):
        self.prog = prog
    ...
    def result2pdb(self):
        ...
        if self.prog == 'AutoDock Vina':
            if glob.glob('*.pdbqt'):
                self.rfiles = glob.glob('*.pdbqt')
            else:
                return
        else:
            if glob.glob('*.dlg'):
                self.rfiles = glob.glob('*.dlg')
            else:
                return
        ...
        if len(self.rfiles) > 1:
            output_file.write('REMARK All docking result from each FILL generated by AutoLigand' + '\n')
