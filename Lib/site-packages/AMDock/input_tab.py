...
class Program_body(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        self.W.process.finished.connect(self.print_finished)
        self.W.process.readyReadStandardOutput.connect(self.output)
        self.W.process.error.connect(self.output_error)
        self.W.process.stateChanged.connect(self.state)
        # signals for functions
        self.W.worker.signals.finished.connect(self.print_finished)
        self.W.worker.signals.state.connect(self.state)

        self.offtarget_docking.toggled.connect(lambda: self.simulation_form(self.offtarget_docking))
        ...
        if self.AMDock.WDIR is None:
            ...
            if reset_opt == QtGui.QMessageBox.Yes:
                ...
                try:
                    ...
                    pass
                if self.AMDock.WDIR is not None:
                    ...
                    if rm_folder == QtGui.QMessageBox.Yes:
                        try:
                            ...
                            shutil.rmtree(self.AMDock.WDIR)
                        except:
    ...
    def simulation_form(self, btn):
        if btn.isChecked():
            if btn.text() == 'Off-Target Docking':
                ...
                self.btnB_lig.show()
                self.btnB_user.show()
                if os.path.exists(`self.AMDock.input_offtarget`):
                    try:
                        os.remove(`self.AMDock.input_offtarget`)
                    except:
                        pass
                self.AMDock.input_offtarget = None
                self.target.name = None
                self.AMDock.ligands = None
                self.target_text.clear()
                self.target_label.clear()
                self.lig_list.hide()
                if os.path.exists(`self.AMDock.input_lig`):
                    try:
                        os.remove(`self.AMDock.input_lig`)
                    except:
                        pass
                self.AMDock.input_lig = None
                self.ligand_text.clear()
                self.ligand_label.clear()
                self.AMDock.ligand_name = None
                if os.path.exists(`self.AMDock.input_target`):
                    try:
                        os.remove(`self.AMDock.input_target`)
                    except:
                        pass
                self.AMDock.input_target = None
                self.AMDock.analog_protein_name = None
                ...
                self.AMDock.analog_protein_name = None
                self.AMDock.analog_ligands = None
                self.offtarget_textB.clear()
                self.offtarget_labelB.clear()
                self.lig_listB.hide()
                self.lig_list.clear()
                self.lig_listB.clear()
                self.AMDock.od = False
                self.btnA_auto.setChecked(True)
                self.btnB_auto.setChecked(True)
                self.offtarget_buttonB.hide()
                self.offtarget_textB.hide()
                self.offtarget_labelB.hide()
                self.offtarget_column_label.hide()
                self.btnB_auto.hide()
                self.btnB_res.hide()
                self.btnB_lig.hide()
        ...
        if self.data is not None:
            ...
            if self.AMDock.program_mode is not 'SCORING':
                ...
                if len(errlist) != 0:
                    ...
                    self.parent.result_tab.prot_label.setText('Target: %s' % self.target.name)
                    self.parent.result_tab.prot_label_sel.setText('%s' % self.target.name)
                    ...
                    if self.AMDock.od:
                        self.parent.result_tab.prot_labelB.setText('Off-Target: %s' % self.AMDock.analog_protein_name)
                        self.parent.result_tab.prot_label_selB.setText('%s' % self.AMDock.analog_protein_name)
                ...
                if self.AMDock.WDIR:
                    ...
                    if self.options == QtGui.QMessageBox.Yes:
                        ...
                        if not self.parent.loader.create_project_function():
                            ...
                            self.proj_loc_label.setText('Project: %s' % self.AMDock.WDIR)
        ...
        if file.objectName() == "target_button":
            if self.target.input is None:
                ...
                if self.prot_opt == QtGui.QMessageBox.Yes:
                    ...
                    self.target_text.clear()
                    if self.AMDock.od:
                        if self.ligand == None and self.target == None:
                            progress(self, 0, 0, 2, reverse=True, mess='Target Definition...')
                        elif self.ligand == None and self.target != None:
            ...
            if self.target:
                if self.AMDock.docking_program == 'AutoDockZn':
                    ...
                    if self.check_opt == QtGui.QMessageBox.Yes:
                        if self.AMDock.analog_metals is None and self.AMDock.input_target is not None:
                            self.AMDock.input_target = None
                            self.target_text.clear()
                            self.target_text.clear()
                            self.target_label.clear()
                            ...
                            self.parent.statusbar.showMessage(self.AMDock.docking_program + " is selected")

        if file.objectName() == "offtarget_button":
            if self.AMDock.input_offtarget is None:
                self.parent.loader.load_proteinB()
            else:
                self.prot_opt = prot_warning(self)
                if self.prot_opt == QtGui.QMessageBox.Yes:
                    os.remove(self.AMDock.input_offtarget)
                    self.AMDock.analog_metals = None
                    self.AMDock.analog_ligands = None
                    self.AMDock.offtarget_prepare = True
                    self.lig_listB.clear()
                    self.lig_listB.hide()
                    self.btnB_lig.show()
                    self.target_labelB.clear()
                    self.target_textB.clear()
                    ...
                    elif self.AMDock.input_lig is None and self.AMDock.input_offtarget is None:
                        progress(self, 0, 0, 5, reverse=True, mess='Off-Target Definition...')
                    elif self.AMDock.input_lig is not None and self.AMDock.input_offtarget is None:
                        progress(self, 0, 0, 4, reverse=True, mess='Off-Target Definition...')
                    else:
                        progress(self, 0, 0, 7, reverse=True, mess='Off-Target Definition...')
                    self.parent.loader.load_proteinB()
            if self.AMDock.input_offtarget:
                if self.AMDock.docking_program == 'AutoDockZn':
                    ...
                    if self.check_opt == QtGui.QMessageBox.Ok:
                        ...
                        if self.AMDock.input_lig == '' and self.AMDock.input_offtarget == None:
                            progress(self, 0, 0, 2, reverse=True, mess='Off-Target Definition...')
                        elif self.AMDock.input_lig == '' and self.AMDock.input_offtarget != None:
                            progress(self, 0, 0, 5, reverse=True, mess='Off-Target Definition...')
                        elif self.AMDock.input_lig != '' and self.AMDock.input_offtarget == None:
                            progress(self, 0, 0, 4, reverse=True, mess='Off-Target Definition...')
                        else:
                    ...
                    if self.check_opt == QtGui.QMessageBox.Yes:
                        if self.AMDock.metals is None and self.AMDock.input_offtarget is not None:
                            self.AMDock.input_offtarget = None
                            self.btnB_lig.show()
                            self.AMDock.analog_metals = None
                            self.AMDock.analog_ligands = None
        ...
        if file.objectName() == "ligand_button":
            if self.AMDock.input_lig is None:
                ...
                if self.lig_opt == QtGui.QMessageBox.Yes:
                    os.remove(self.AMDock.input_lig)
                    self.AMDock.ligand_prepare = True
                    self.AMDock.ligand_pdbqt = None
                    self.ligand_text.clear()
                    self.ligand_label.clear()
                    if self.AMDock.od:
                        ...
                        if self.AMDock.input_offtarget == None:
    ...
    def prepare_receptor(self):
        # move to input folder in project location
        ...
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP DOCKING_PROGRAM: %s' %
                                                              self.AMDock.docking_program)
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP MODE: %s' % self.AMDock.program_mode)
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP TARGET_PROTEIN: %s' %
                                                              self.target.name)
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP    Hetero: %s' % self.target_info.get_het())
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP    Zn atoms: %s' % self.target_info.get_zn())
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP OFF-TARGET_PROTEIN: %s' % self.offtarget.name)
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP    Hetero: %s' % self.offtarget_info.get_het())
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP    Zn atoms: %s' %
                                                              self.offtarget_info.get_zn())
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP LIGAND: %s' % self.ligand.name)
        self.parent.configuration_tab.log_wdw.textedit.append('AMDOCK: IP    heavy_atoms: %s' %
        ...
        if self.AMDock.target_prepare:
            self.AMDock.protein_pqr = self.target.name + '_h.pqr'
            self.AMDock.protein_h = self.target.name + '_h.pdb'
            self.AMDock.protein_pdbqt = self.target.name + '_h.pdbqt'
        if self.AMDock.ligand_prepare:
            self.AMDock.ligand_h = self.AMDock.ligand_name + "_h.pdb"
            self.AMDock.ligand_pdbqt = self.AMDock.ligand_name + '_h.pdbqt'
        if self.AMDock.od:
            if self.AMDock.offtarget_prepare:
                self.AMDock.analog_protein_pqr = self.AMDock.analog_protein_name + '_h.pqr'
                self.AMDock.analog_protein_h = self.AMDock.analog_protein_name + '_h.pdb'
                self.AMDock.analog_protein_pdbqt = self.AMDock.analog_protein_name + '_h.pdbqt'
        self.list_process = []
        if self.AMDock.target_prepare:
            self.pdb2pqr = {'PDB2PQR': [self.ws.this_python, [self.ws.pdb2pqr_py, '--ph-calc-method=propka',
                                                              '--verbose', '--noopt', '--drop-water', '--chain',
                                                              '--with-ph', str(self.AMDock.pH),
                                                              '--ff=%s' % self.AMDock.forcefield,
                                                              self.AMDock.protein_file,
                                                              self.AMDock.protein_pqr]]}
            if self.AMDock.metals is not None:
                fix_pqr_arg = [self.AMDock.protein_file, self.AMDock.protein_pqr, True]
            else:
                fix_pqr_arg = [self.AMDock.protein_file, self.AMDock.protein_pqr]
            self.fix_pqr = {'function Fix_PQR': [Fix_PQR, fix_pqr_arg]}

            prepare_receptor4_arg = [self.ws.prepare_receptor4_py, '-r', self.AMDock.protein_h, '-v', '-U',
        ...
        if self.AMDock.offtarget_prepare:
            self.pdb2pqrB = {'PDB2PQR B': [self.ws.this_python, [self.ws.pdb2pqr_py, '--ph-calc-method=propka',
                                                                 ...
                                                                 '--with-ph', str(self.AMDock.pH),
                                                                 '--ff=%s' % self.AMDock.forcefield,
                                                                 self.AMDock.analog_protein_file,
                                                                 self.AMDock.analog_protein_pqr]]}
            if self.AMDock.analog_metals:
                fix_pqr_argB = [self.AMDock.analog_protein_file, self.AMDock.analog_protein_pqr, True]
            else:
                fix_pqr_argB = [self.AMDock.analog_protein_file, self.AMDock.analog_protein_pqr]

            self.fix_pqrB = {'function Fix_PQR B': [Fix_PQR, fix_pqr_argB]}
            prepare_receptor4_argB = [self.ws.prepare_receptor4_py, '-r', self.AMDock.analog_protein_h, '-v', '-U',
                                      'nphs_lps_waters_nonstdres_deleteAltB']
            self.prepare_receptor4B = {'Prepare_Receptor4 B': [self.ws.this_python, prepare_receptor4_argB]}
        if self.AMDock.ligand_prepare:
            protonate_ligand_arg = ['-i', 'pdb', self.AMDock.ligand_pdb, '-opdb', '-O', self.AMDock.ligand_h, '-h',
                                    '-p', `self.AMDock.pH`]
            self.protonate_ligand = {'Protonate Ligand': [self.AMDock.openbabel, protonate_ligand_arg]}

            prepare_ligand4_arg = [self.ws.prepare_ligand4_py, '-l', self.AMDock.ligand_h, '-v', '-o',
                                   self.AMDock.ligand_pdbqt]
        ...
        if self.AMDock.od:
            ...
            if self.AMDock.ligand_prepare:
                self.list_process.append(self.protonate_ligand)
                self.list_process.append(self.prepare_ligand4)
        if self.AMDock.scoring and self.AMDock.docking_program != 'AutoDock Vina':
            prev_ligand_arg = [self.AMDock.protein_file, None, None, self.AMDock.gd, False]
            print prev_ligand_arg
            ...
            self.worker.start_process()
            self._size_x = self._size_y = self._size_z = self.AMDock.rg
        self.queue = Queue.Queue()
    ...
    def go_result(self):
        ...
        self.parent.result_tab.import_text.setText(self.AMDock.amdock_file)
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            ...
            self.re = Result_Analysis(vina_result, 'all_poses_target_ADV_result.pdb',
                                      os.path.join(self.AMDock.input_dir, self.AMDock.protein_pdbqt),
                                      self.AMDock.heavy_atoms)
        ...
        if self.AMDock.od:
            ...
            if self.AMDock.docking_program == 'AutoDock Vina':
                ...
                self.reB = Result_Analysis(vina_resultB, 'all_poses_off-target_ADV_result.pdb',
                                           os.path.join(self.AMDock.input_dir, self.AMDock.analog_protein_pdbqt),
                                           self.AMDock.heavy_atoms)
    ...
    def scoring(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            ...
            prepare_gpf4_arg = [self.ws.prepare_gpf4_py, '-l', str(self.AMDock.ligand_pdbqt), '-r',
            ...
            prepare_dpf_arg = [self.ws.prepare_dpf_py, '-l', str(self.AMDock.ligand_pdbqt), '-r',
            ...
            pseudozn_arg = [self.ws.zinc_pseudo_py, '-r', str(self.AMDock.protein_pdbqt)]
            self.pseudozn = {'PseudoZn': [self.ws.this_python, pseudozn_arg]}
            prepare_gpf4zn_arg = [self.ws.prepare_gpf4zn_py, '-l', str(self.AMDock.ligand_pdbqt), '-r',
            ...
            prepare_dpfzn_arg = [self.ws.prepare_dpf_py, '-l', str(self.AMDock.ligand_pdbqt), '-r', protein_TZ, '-e']
    ...
    def binding_site(self):
        ...
        self.reset_button.setEnabled(False)
        self.grid_box.setEnabled(False)
        self._size_x = self._size_y = self._size_z = self._size_xB = self._size_yB = self._size_zB = self.AMDock.rg
        self.AMDock.FILL = 'FILL_%dout1.pdb' % (6 * self.AMDock.heavy_atoms)
        self.AMDock.selected_residues = self.grid_predef_text.text()
        self.AMDock.analog_selected_residues = self.grid_predef_textB.text()
        process_list = []
        if self.AMDock.grid_def == 'auto':
            ...
            self.autogrid4 = {'AutoGrid4': [self.ws.autogrid, autogrid_arg]}
            autoligand_arg = [str(self.ws.autoligand_py), '-r', str(self.AMDock.protein_pdbqt[:-6]), '-a',
            ...
            if self.need_grid:
                process_list.extend(self.list_process)
        elif self.AMDock.grid_def == 'by_user':
            if self.need_grid:
        ...
        if self.AMDock.od:
            if self.AMDock.analog_grid_def == 'auto':
                ...
                prepare_gpf4_argB = [self.ws.prepare_gpf4_py, '-l', str(self.AMDock.ligand_pdbqt), '-r',
                ...
                autoligand_argB = [str(self.ws.autoligand_py), '-r', str(self.AMDock.analog_protein_pdbqt[:-6]), '-a',
                                   str(self.AMDock.heavy_atoms)]
                ...
                autoligand_arg = [str(self.ws.autoligand_py), '-r', str(self.AMDock.analog_protein_pdbqt[:-6]), '-a',
                ...
                if self.need_gridB:
                    process_list.extend(self.list_process)
            elif self.AMDock.analog_grid_def == 'by_user':
                if self.need_gridB:
    ...
    def start_docking_prog(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            ...
            if self.AMDock.od:
                ...
                prepare_dpf_argB = [self.ws.prepare_dpf_py, '-l', str(self.AMDock.ligand_pdbqt), '-r',
            ...
            prepare_dpfzn_arg = [self.ws.prepare_dpf_py, '-l', str(self.AMDock.ligand_pdbqt), '-r', protein_TZ, '-p',
            ...
            if self.AMDock.od:
                ...
                pseudozn_argB = [self.ws.zinc_pseudo_py, '-r', str(self.AMDock.analog_protein_pdbqt)]
                self.pseudoznB = {'PseudoZn B': [self.ws.this_python, pseudozn_argB]}

                prepare_gpf4zn_argB = [self.ws.prepare_gpf4zn_py, '-l', str(self.AMDock.ligand_pdbqt), '-r',
                ...
                prepare_dpfzn_argB = [self.ws.prepare_dpf_py, '-l', str(self.AMDock.ligand_pdbqt), '-r', proteinB_TZ,
