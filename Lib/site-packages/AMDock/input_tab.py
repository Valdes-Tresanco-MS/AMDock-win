...
#         # self.grid_box.setEnabled(False)
    ...
    def binding_site(self):
        ...
        self._size_x = self._size_y = self._size_z = self._size_xB = self._size_yB = self._size_zB = \
            self.ligand_info.get_gyrate()
        ...
        if self.AMDock.project.bsd_mode == 0:
            ...
            if self.AMDock.project.mode == 1:
                ...
                prepare_gpf4B = {'Prepare_gpf4 B': [self.AMDock.this_python, [self.AMDock.prepare_gpf4_py, '-l',
                                self.ligand.pdbqt, '-r', self.offtarget.pdbqt, '-p', 'npts={0},{1},{2}'.format(
                        *self.offtarget_info.size), '-p', 'gridcenter={0},{1},{2}'.format(*self.offtarget_info.center),
                        '-p', 'spacing=%.3f' % self.AMDock.spacing_autoligand, '-o', self.offtarget.auto_lig]]}
            ...
            if self.AMDock.project.mode == 1:
                ...
                prepare_gpf4B = {'Prepare_gpf4 B': [self.AMDock.this_python, [self.AMDock.prepare_gpf4_py, '-l',
                                 self.ligand.pdbqt, '-r',  self.offtarget.pdbqt, '-p', 'npts={0},{1},{2}'.format(
                                 *self.offtarget_info.size), '-p', 'gridcenter={0},{1},{2}'.format(
                                 *self.offtarget_info.center), '-p', 'spacing=%.3f' % self.AMDock.spacing_autoligand,
                                 '-o', self.offtarget.auto_lig]]}
                queue.put(prepare_gpf4B)
                autogrid4B = {'AutoGrid4 B': [self.AMDock.autogrid, ['-p', self.offtarget.auto_lig]]}
                queue.put(autogrid4B)
                autoligandB = {'AutoLigand B': [self.AMDock.this_python, [self.AMDock.autoligand_py, '-r',
                               self.offtarget.pdbqt_name, '-a', '{}'.format(self.ligand_info.get_ha()), '-x',
                               self.target_info.selection_center[0],  '-y', self.target_info.selection_center[1], '-z',
                               self.target_info.selection_center[2], '-f', '1']]}
                queue.put(autoligandB)
    ...
    def fill_selection(self, autoligand_table):
        if autoligand_table.objectName() == 'autoligand_target':
            ...
            print fill_info.center
            if fill_info.center:
                self.grid_center = fill_info.center
                print self.grid_center
            else:
                for item in items:
    ...
    def start_docking_prog(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            vina_arg = ['--receptor', self.target.pdbqt, '--ligand', self.ligand.pdbqt, '--center_x',
                        '{0:.2f}'.format(self.grid_center[0]), '--center_y', '{0:.2f}'.format(self.grid_center[1]),
                        '--center_z', '{0:.2f}'.format(self.grid_center[2]), '--size_x', '{0:.2f}'.format(
                        self._size_x),'--size_y','{0:.2f}'.format(self._size_y), '--size_z', '{0:.2f}'.format(
                        self._size_z), '--cpu', str(self.AMDock.ncpu), '--num_modes', str(self.AMDock.poses_vina),
            ...
            if self.AMDock.project.mode == 1:
                vina_argB = ['--receptor', self.offtarget.pdbqt, '--ligand', self.ligand.pdbqt,'--center_x',
                            '{0:.2f}'.format(self.grid_centerB[0]), '--center_y', '{0:.2f}'.format(self.grid_centerB[1]),
                            '--center_z', '{0:.2f}'.format(self.grid_centerB[2]), '--size_x', '{0:.2f}'.format(
                            self._size_x),'--size_y','{0:.2f}'.format(self._size_y), '--size_z', '{0:.2f}'.format(
                            self._size_z), '--cpu', str(self.AMDock.ncpu), '--num_modes', str(self.AMDock.poses_vina),
                            '--exhaustiveness', str(self.AMDock.exhaustiveness), '--out', os.path.join(
            ...
            prepare_gpf4_arg = [self.AMDock.prepare_gpf4_py, '-l', self.ligand.pdbqt, '-r',
                                ...
                                'npts=%d,%d,%d' % (self._size_x / self.AMDock.spacing_autodock, self._size_y /
                                                   self.AMDock.spacing_autodock, self._size_z /
            ...
            if self.AMDock.project.mode == 1:
                prepare_gpf4_argB = [self.AMDock.prepare_gpf4_py, '-l', self.ligand.pdbqt, '-r',
                                ...
                                'npts=%d,%d,%d' % (self._size_xB / self.AMDock.spacing_autodock, self._size_yB /
                                                   self.AMDock.spacing_autodock, self._size_zB /
                                                   self.AMDock.spacing_autodock), '-p', 'gridcenter={0},{1},'
            ...
            if self.AMDock.project.mode == 1:
                ...
                prepare_gpf4zn_argB = [self.AMDock.prepare_gpf4zn_py, '-l', self.ligand.pdbqt, '-r',
                                      ...
                                      'npts=%d,%d,%d' % (self._size_xB / self.AMDock.spacing_autodock, self._size_yB /
                                      self.AMDock.spacing_autodock, self._size_zB / self.AMDock.spacing_autodock),
