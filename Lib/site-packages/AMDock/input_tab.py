    ...
    def binding_site(self):
        ...
        if self.AMDock.project.bsd_mode == 0:
            ...
            if self.AMDock.project.mode == 1:
                ...
                prepare_gpf4B = {'Prepare_gpf4': [self.AMDock.this_python, [self.AMDock.prepare_gpf4_py, '-l',
                                self.ligand.pdbqt, '-r', self.offtarget.pdbqt, '-p', 'npts={0},{1},{2}'.format(
                        *self.target_info.size), '-p', 'gridcenter={0},{1},{2}'.format(*self.offtarget_info.center),
                        '-p', 'spacing=%.3f' % self.AMDock.spacing_autoligand, '-o', self.offtarget.auto_lig]]}
                queue.put(prepare_gpf4B)
                autogrid4B = {'AutoGrid4': [self.AMDock.autogrid, ['-p', self.offtarget.auto_lig]]}
                queue.put(autogrid4B)
                autoligandB = {'AutoLigand': [self.AMDock.this_python, [self.AMDock.autoligand_py, '-r',
