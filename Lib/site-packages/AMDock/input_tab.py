...
#         # self.grid_box.setEnabled(False)
        ...
        self.W.process.finished.connect(self.run_queue)
    ...
    def output_function(self, result):
        print result
    ...
    def run_queue(self, exitcode, exitstatus):
        print exitcode
        if not exitcode:
    ...
    def load_file(self, file):  # ok
        ...
        if file.objectName() == "wdir_button":
            self.wdir_loc = self.AMDock.loader.project_location()
            if self.wdir_loc:
...
#                 # self.create_project.setEnabled(True)
        ...
        if file.objectName() == "offtarget_button":
            ...
            if self.AMDock.input_offtarget:
                if self.AMDock.docking_program == 'AutoDockZn':
                    self.check_opt = self.AMDock.checker.autodockzn_check('B')
                    ...
                    self.check_opt = self.AMDock.checker.check_correct_prog('B')
                    if self.check_opt == QtGui.QMessageBox.Yes:
                        if self.AMDock.metals is None and self.AMDock.input_offtarget is not None:
                            ...
                            self.AMDock.statusbar.showMessage(self.AMDock.docking_program + " is selected")
                ...
                if self.lig_opt == QtGui.QMessageBox.Yes:
                    ...
                    if self.AMDock.od:
                        if self.AMDock.input_offtarget == None and self.AMDock.input_target == None:
                            progress(self.AMDock.program_body, 0, 0, 2, reverse=True, mess='Ligand Definition...')
                        elif self.AMDock.input_offtarget == None and self.AMDock.input_target != None:
                            progress(self.AMDock.program_body, 0, 0, 5, reverse=True, mess='Ligand Definition...')
                        elif self.AMDock.input_offtarget != None and self.AMDock.input_target == None:
                            progress(self.AMDock.program_body, 0, 0, 5, reverse=True, mess='Ligand Definition...')
                        else:
                            progress(self.AMDock.program_body, 0, 0, 8, reverse=True, mess='Ligand Definition...')
                    else:
    ...
    def prepare_receptor(self):
        ...
        if not self.AMDock.loc_project:
            msg = QtGui.QMessageBox.critical(self.AMDock, 'Error', 'The project location most be defined',
                                             QtGui.QMessageBox.Ok)
            return
...
#         # self.reset_button.setEnabled(False)
        ...
        if self.AMDock.od:
            ...
            self.AMDock.configuration_tab.log_wdw.textedit.append('AMDOCK: IP    Hetero: %s' % self.offtarget_info.get_het())
        ...
        if self.target.prepare:
            pdb2pqr = {'PDB2PQR': [self.AMDock.this_python, [self.AMDock.pdb2pqr_py, '--ph-calc-method=propka',
                                                              ...
                                                              self.target.input, self.target.name + '_h.pqr']]}
            fix_pqr = {'function Fix_PQR': [Fix_PQR, [self.target.input, self.target.name + '_h.pqr',
                                                           self.target.zn_atoms]]}

            prepare_receptor4 = {'Prepare_Receptor4': [self.AMDock.this_python, [self.AMDock.prepare_receptor4_py,
                                                                        '-r', self.target.name + '_h.pdb', '-v', '-U',
        ...
        if self.AMDock.od:
            if self.offtarget.prepare:
                pdb2pqrB = {'PDB2PQR B': [self.AMDock.this_python, [self.AMDock.pdb2pqr_py, '--ph-calc-method=propka',
                                                                     ...
                                                                     '--with-ph', str(self.pH_value.value()),
                                                                     '--ff=%s' % self.AMDock.configuration_tab.forcefield,
                                                                     self.offtarget.input, self.offtarget.name + '_h.pqr']]}

                fix_pqrB = {'function Fix_PQR B': [Fix_PQR, self.offtarget.input, self.offtarget.name + '_h.pqr',
                                                        self.offtarget.zn_atoms]}
                prepare_receptor4B = {'Prepare_Receptor4 B': [self.AMDock.this_python,
                                        [self.AMDock.prepare_receptor4_py, '-r', self.offtarget.name + '_h.pdb', '-v', '-U',
                                          'nphs_lps_waters_nonstdres_deleteAltB']]}
    ...
    def binding_site(self):
        ...
        if self.AMDock.state == -2:
            msg = QtGui.QMessageBox.critical(self.AMDock, 'Error', 'Other processes are running in the background. '
                                             ...
                                             QtGui.QMessageBox.Ok)
            return
        elif self.AMDock.state in [-1, 0, 1]:
            msg = QtGui.QMessageBox.critical(self.AMDock, 'Error', 'It seems that not all previous steps have been '
                                             ...
                                             QtGui.QMessageBox.Ok)
            return
        elif self.AMDock.state == 3:
            msg = QtGui.QMessageBox.warning(self.AMDock, 'Warning', 'It seems that he has already carried out this '
                                                                    'process and is in the realization of the '
                                                                    'molecular docking. Do you wish to continue?',
                                            QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
