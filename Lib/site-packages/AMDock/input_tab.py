...
#         # self.grid_box.setEnabled(False)
    ...
    def prepare_receptor(self):
        print self.AMDock.state, self.AMDock.section
        ...
        if self.AMDock.project.mode and self.offtarget_info.get_het():
        ...
        if self.target.prepare:
            ...
            fix_pqr = {'function Fix_PQR': [Fix_PQR, [self.target.input, self.target.pqr, self.target.zn_atoms]]}
        ...
        if self.AMDock.project.mode == 1:
            if self.offtarget.prepare:
                ...
                fix_pqrB = {'function Fix_PQR B': [Fix_PQR, self.offtarget.input, self.offtarget.pqr,
