...
#         # self.grid_box.setEnabled(False)
                ...
                if self.need_gridB:
                    ...
                    if self.size_xB.value() < self.size or self.size_yB.value() < self.size or \
                        ...
                        self.grid_optB, self.dim_listB = smallbox_warning(self, {'x': self.size_x.value(),
                                                                                 'y': self.size_y.value(),
                                                                                 'z': self.size_z.value()},
                                                                          self.size, self.AMDock.offtarget.name)
    ...
    def fill_selection(self, autoligand_table):
        ...
        #     fill_info = PDBINFO(
        #         'FILL_{}_{}out{}.pdb'.format(self.AMDock.offtarget.pdbqt_name, self.ligand_info.get_ha() * 6, row))
    ...
    def autodock_output(self):
        if self.AMDock.project.prog == 'AutoDock4':
            self.ADout_file = os.path.join(self.AMDock.project.results, self.AMDock.ligand.pdbqt_name + '_' + self.AMDock.target.dlg)
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            ...
            if self.AMDock.project.mode == 2:
                ...
                prepare_dfp4zn = {'Prepare_dpf4': [self.AMDock.this_python, [self.AMDock.prepare_dpf_py, '-l',
                                                  self.AMDock.ligand.pdbqt, '-r', self.AMDock.target.tzpdbqt, '-e']]}
                ...
                prepare_dfp4zn = {'Prepare_dpf4': [self.AMDock.this_python, [self.AMDock.prepare_dpf_py, '-l',
                                       self.AMDock.ligand.pdbqt, '-r', self.AMDock.target.tzpdbqt, '-p', 'rmstol=%s' % self.AMDock.rmsd,
                                     '-p', 'ga_num_evals=%s' % self.AMDock.eval, '-p', 'ga_run=%s' % self.AMDock.runs]]}
                queue.put(prepare_dfp4zn)

                autodockzn_arg = ['-p', self.AMDock.ligand.pdbqt_name + '_' + self.AMDock.target.tzdpf, '-l', os.path.join(
                    self.AMDock.project.results, self.AMDock.ligand.pdbqt_name + '_' + self.AMDock.target.tzdlg)]
                autodockzn = {'AutoDock4ZN': [self.AMDock.autodock, autodockzn_arg]}
                queue.put(autodockzn)
                ...
                if self.AMDock.project.mode == 1:
                    ...
                    prepare_dfp4znB = {'Prepare_dpf4 B': [self.AMDock.this_python, [self.AMDock.prepare_dpf_py, '-l',
                                        self.AMDock.ligand.pdbqt, '-r', self.AMDock.offtarget.tzpdbqt, '-p', 'rmstol=%s' %
                                        self.AMDock.rmsd, '-p', 'ga_num_evals=%s' % self.AMDock.eval, '-p', 'ga_run=%s' %
                                        self.AMDock.runs]]}
                    queue.put(prepare_dfp4znB)

                    autodockzn_argB = ['-p', self.AMDock.ligand.pdbqt_name + '_' + self.AMDock.offtarget.tzdpf, '-l', os.path.join(
                        self.AMDock.project.results, self.AMDock.ligand.pdbqt_name + '_' + self.AMDock.offtarget.tzdlg)]
