...
#         # self.grid_box.setEnabled(False)
        ...
        self.section_state.setStyleSheet("QLabel {font-weight: bold;}")
...
#         #         self.grid_icon_ok.show()
    ...
    def autoligand_out(self, mol):
        ...
        for line in ofile:
            line = line.strip('\n')
            mol.fill_list.append([line.split()[6].strip(','), line.split()[13]])
    ...
    def fill_selection(self, autoligand_table):
        if autoligand_table.objectName() == 'autoligand_target':
            ...
            fill_info = PDBINFO('FILL_{}_{}out{}.pdb'.format(self.AMDock.offtarget.pdbqt_name, self.AMDock.ligand.ha
                                                             * 6, row))
            if fill_info.center:
    ...
    def for_finished(self, info):
        ...
        if exitcode:
            error_warning(self,prog_name,exitcode)
            return
        ...
        if self.AMDock.section == 1:
            ...
            if self.AMDock.project.bsd_mode_offtarget in [0, 1]:
                if prog_name == 'Prepare_gpf4 B':
                    ...
                    self.AMDock.log_widget.textedit.append('AMDOCK: BSD > Generate GPF file for Off-Target Protein... '
                                                           'Done.')
            ...
            if prog_name == 'Prepare_gpf4 B':
                if self.AMDock.project.bsd_mode_offtarget == 0:
                    ...
                    self.progress(60, 0, 'Generate GPF file...',1)
                # self.AMDock.log_widget.textedit.append(
                #     'AMDOCK: MD > Generate GPF file for Off-Target Protein... Done.')
            if prog_name == 'AutoGrid4':
                if self.AMDock.project.bsd_mode_target == 0:
                    self.progress(50/len(self.AMDock.target.fill_list), self.AMDock.project.mode, 'Running AutoGrid4 for Target...')
                else:
            ...
            if prog_name == 'AutoGrid4 B':
                if self.AMDock.project.bsd_mode_offtarget == 0:
                    self.progress(75/ len(self.AMDock.offtarget.fill_list), 0, 'Running AutoGrid4 for Off-Target...')
                else:
            ...
            if prog_name == 'AutoDock4ZN':
                ...
                if self.AMDock.project.bsd_mode_target == 0:
                    self.progress(100 / len(self.AMDock.target.fill_list), self.AMDock.project.mode,
                                  'Determining better poses for Target...', 1)
                else:
                    self.progress(100, self.AMDock.project.mode, 'Determining better poses for Target...', 1)
    ...
    def pymol_readStdOutput(self):
        ...
        if re.search('AMDock INFO', self.pymol_out):
            ...
            if prot == 'target':
                ...
                self.grid_center = [str(self.coor_x.value()), str(self.coor_y.value()), str(self.coor_z.value())]
                self.size = [str(self.size_x.value()), str(self.size_y.value()), str(self.size_z.value())]
                ...
                self.sizeB = [str(self.size_xB.value()), str(self.size_yB.value()), str(self.size_zB.value())]
            self.AMDock.section = 1
            self.progressBar_global.setValue(40)
...
#         # self.AMDock.result_tab.load_button.setEnabled(False)
        ...
        self.re = Result_Analysis(self.AMDock.docking_program , self.AMDock.target, os.path.join(
                self.AMDock.project.input, self.AMDock.target.pdbqt), self.AMDock.ligand.ha)
        ...
        if self.AMDock.project.mode == 1:
            ...
            self.reB = Result_Analysis(self.AMDock.docking_program, self.AMDock.offtarget, os.path.join(
                self.AMDock.project.input, self.AMDock.offtarget.pdbqt), self.AMDock.ligand.ha)
            ...
            if self.AMDock.project.mode == 2:
                ...
                if self.AMDock.project.bsd_mode_target == 0:
                    for nfill in range(len(self.AMDock.target.fill_list)):
                        fill_info = PDBINFO('FILL_{}_{}out{}.pdb'.format(self.AMDock.target.pdbqt_name,
    ...
    def init_prog(self, prog):
        ...
        if prog == 'AutoLigand':
            self.AMDock.log_widget.textedit.append('AMDOCK: BSD > Running AutoLigand for Target Protein...')
        if prog == 'AutoLigand B':
            self.AMDock.log_widget.textedit.append('AMDOCK: BSD > Running AutoLigand for Off-Target Protein...')
        if prog == 'Prepare_gpf4':
            self.AMDock.log_widget.textedit.append('AMDOCK: BSD > Running Prepare_gpf for Target Protein...')
        if prog == 'Prepare_gpf4 B':
            self.AMDock.log_widget.textedit.append('AMDOCK: BSD > Running Prepare_gpf for Off-Target Protein...')
    ...
    def highlight(self):
        ...
        self.p1.setStyleSheet("QLabel { color : black; font-weight: bold;}")
        self.p2.setStyleSheet("QLabel { color : black; font-weight: bold;}")
        self.p3.setStyleSheet("QLabel { color : black; font-weight: bold;}")
        self.p4.setStyleSheet("QLabel { color : black; font-weight: bold;}")
        self.p5.setStyleSheet("QLabel { color : black; font-weight: bold;}")

        if self.AMDock.section == -1:
            self.p1.setStyleSheet("QLabel { background-color : blue; color : white; font-weight: bold;}")
        if self.AMDock.section == 0:
            self.p2.setStyleSheet("QLabel { background-color : blue; color : white; font-weight: bold;}")
        elif self.AMDock.section == 1:
            self.p3.setStyleSheet("QLabel { background-color : blue; color : white; font-weight: bold;}")
            self.p5.setStyleSheet("QLabel { background-color : blue; color : white; font-weight: bold;}")
        elif self.AMDock.section == 2:
            self.p4.setStyleSheet("QLabel { background-color : blue; color : white; font-weight: bold;}")
        # elif self.AMDock.section == 3:
