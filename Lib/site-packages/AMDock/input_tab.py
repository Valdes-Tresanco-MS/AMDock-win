...
#         # self.grid_box.setEnabled(False)
        ...
        # self.p1.setStyleSheet("QLabel { background-color : blue; color : white; font-weight: bold;}")
        # self.p2 = QtGui.QLabel('Input Files >')
        # self.p2.setStyleSheet("QLabel {font-weight: bold;}")
        # self.p3 = QtGui.QLabel('Search Space >')
        # self.p3.setStyleSheet("QLabel {font-weight: bold;}")
        # self.p3.setFont(font3)
        # self.p4 = QtGui.QLabel('Docking')
        # self.p4.setStyleSheet("QLabel {font-weight: bold;}")
        # self.p5 = QtGui.QLabel('Scoring')
        # self.p5.setStyleSheet("QLabel {font-weight: bold;}")
        # self.p5.hide()
        ...
        # self.ind_layout.addWidget(self.p5)
        ...
        # self.all_options.addWidget(self.grid_pymol_button, 9, 1, 2, 1, QtCore.Qt.AlignCenter)
    ...
    def prepare_receptor(self):
        ...
        self.list_process = []
        # Align proteins
        if self.AMDock.prot_align and self.AMDock.project.mode == 1:
            if self.AMDock.target.ext in ['pdb', 'ent'] and self.AMDock.offtarget.ext in ['pdb', 'ent']:
                align = {'Align': [self.AMDock.this_python, [self.AMDock.pymol, '-c', self.AMDock.aln_pymol, '--', '-t',
                                                             self.AMDock.target.input, '-o',
                                                             self.AMDock.offtarget.input]]}
                self.list_process.append(align)
    ...
    def binding_site(self):
        ...
        if self.AMDock.project.bsd_mode_target == 0:
            ...
            prepare_gpf4 = {'Prepare_gpf4': [self.AMDock.this_python, [self.AMDock.prepare_gpf4_py, '-l',
                            self.AMDock.ligand.pdbqt, '-r', self.AMDock.target.pdbqt, '-p', 'npts={0},{1},{2}'.format(
                            *self.target_info.size), '-p', 'gridcenter={0},{1},{2}'.format(*self.grid_center),
                ...
                if self.need_gridB:
                    ...
                    if self.size_xB.value() < self.lig_size or self.size_yB.value() < self.lig_size or \
                        ...
                        if self.grid_optB == QtGui.QMessageBox.Yes:
                            ...
                            self.sizeB = [int(self.size_xB.value()), int(self.size_yB.value()),
                                          int(self.size_zB.value())]
...
#         #         self.grid_icon_ok.show()
    ...
    def grid_actions(self, btn):
        ...
        if self.AMDock.project.bsd_mode_target == 2:
            ...
            visual_arg.extend(['--f_rep_type', '1', '--f_ligands', os.path.join(self.AMDock.project.input,
                           "FILL_%s_%sout01.pdb" % (self.AMDock.target.pdbqt_name, self.AMDock.ligand.ha * 6)),
        ...
        #                   '-s', '{},{},{}'.format(*self.sizeB), '-p', 'off-target']
    ...
    def autoligand_out(self, mol):
        ...
        for line in ofile:
            ...
            if not fill_info.center:
                ...
                continue
            mol.fill_list['%s' % fill] = [line.split()[6].strip(','), line.split()[13], fill_info.center]
            self.AMDock.log_widget.textedit.append(Ft('FILL: %s  Vol: %s  Energy: %s  Center: %s' %
                                                      (fill, line.split()[6].strip(','), line.split()[13],
                                                       fill_info.center)).definitions())
            fill += 1
    ...
    def fill_selection(self, autoligand_table):
        if autoligand_table.objectName() == 'autoligand_target':
            ...
            fill_info = PDBINFO('FILL_{}_{}out{:02d}.pdb'.format(self.AMDock.target.pdbqt_name, self.AMDock.ligand.ha*6, row))
            if fill_info.center:
    ...
    def for_finished(self, info):
        ...
        if prog_name == 'AutoLigand':
            ...
            if self.AMDock.target.fill_list:
                ...
                fill_info = PDBINFO(
                    'FILL_{}_{}out01.pdb'.format(self.AMDock.target.pdbqt_name, self.AMDock.ligand.ha * 6))
            ...
            fill_info = PDBINFO('FILL_{}_{}out01.pdb'.format(self.AMDock.target.pdbqt_name, self.AMDock.ligand.ha * 6))
            # print fill_info, fill_info.center, '#####'
            if fill_info.center:
                self.grid_center = [str(x) for x in fill_info.center]
            else:
            ...
            if self.AMDock.offtarget.fill_list:
                ...
                fill_info = PDBINFO(
                    'FILL_{}_{}out01.pdb'.format(self.AMDock.offtarget.pdbqt_name, self.AMDock.ligand.ha *
                                                6))
                # print fill_info, fill_info.center, '#####'
                if fill_info.center:
                    self.grid_centerB = [str(x) for x in fill_info.center]
            ...
            fill_info = PDBINFO('FILL_{}_{}out01.pdb'.format(self.AMDock.offtarget.pdbqt_name, self.AMDock.ligand.ha *
                                                            6))
            # print fill_info, fill_info.center, '#####'
            if fill_info.center:
                self.grid_centerB = [str(x) for x in fill_info.center]
        ...
        if self.AMDock.section == 1:
            if self.AMDock.project.bsd_mode_target in [0, 1]:
                if prog_name == 'Prepare_gpf4':
                    self.progress(20, self.AMDock.project.mode, 'Generate Target GPF file...', 1)
                ...
                if prog_name == 'AutoGrid4':
                    ...
                    self.AMDock.log_widget.textedit.append(Ft('Running AutoGrid4 for Target Protein... Done.').process())
            ...
            if self.AMDock.project.bsd_mode_offtarget in [0, 1]:
                if prog_name == 'Prepare_gpf4 B':
                    self.progress(10, 0, 'Generate Off-Target GPF file...', 1)
            ...
            if prog_name == 'AutoDock Vina':
                ...
                # self.AMDock.log_widget.textedit.append('AMDOCK: MD > Determination of better poses for Target '
                #                                        'Protein... Done.')
            ...
            if prog_name == 'Prepare_gpf4':
                if self.AMDock.project.bsd_mode_target == 0:
                    self.progress(10/len(self.AMDock.target.fill_list), self.AMDock.project.mode, 'Generate GPF file...')
                else:
            ...
            if prog_name == 'Prepare_gpf4 B':
                if self.AMDock.project.bsd_mode_offtarget == 0:
                    self.progress(10/len(self.AMDock.offtarget.fill_list), 0, 'Generate GPF file...')
                else:
            ...
            if prog_name == 'AutoGrid4':
                if self.AMDock.project.bsd_mode_target == 0:
                    self.progress(30/len(self.AMDock.target.fill_list), self.AMDock.project.mode, 'Running AutoGrid4 '
                                                                                                  'for Target...')
                else:
            ...
            if prog_name == 'AutoGrid4 B':
                if self.AMDock.project.bsd_mode_offtarget == 0:
                    self.progress(30/ len(self.AMDock.offtarget.fill_list), 0, 'Running AutoGrid4 for Off-Target...')
                else:
                    self.progress(70, 0, 'Running AutoGrid4 for Off-Target...', 1)

                # self.AMDock.log_widget.textedit.append(
                #     'AMDOCK: MD > Running AutoGrid4 for Off-Target Protein... Done.')
            if prog_name == 'Prepare_dpf4':
                if self.AMDock.project.bsd_mode_target == 0:
                    self.progress(10/ len(self.AMDock.target.fill_list), self.AMDock.project.mode, 'Generate DPF file...')
                else:
                    self.progress(50, self.AMDock.project.mode, 'Generate DPF file...', 1)
                # self.AMDock.log_widget.textedit.append(
                #     'AMDOCK: MD > Generate DPF file for Target Protein... Done.')
            if prog_name == 'Prepare_dpf4 B':
                if self.AMDock.project.bsd_mode_offtarget == 0:
                    self.progress(10/len(self.AMDock.offtarget.fill_list), 0, 'Generate DPF file...')
                else:
                    self.progress(75, 0, 'Generate DPF file...', 1)
                # self.AMDock.log_widget.textedit.append(
        ...
        if self.AMDock.project.prog == 'AutoGrid4':
            ...
            if re.search('%', self.out):
                ...
                if self.AMDock.project.bsd_mode_target == 0 and self.AMDock.section == 2:
                    ...
                    print '#########', current/len(self.AMDock.target.fill_list)
                else:
    ...
    def autodock_output(self):
        # if self.AMDock.project.bsd_mode_target == 0 or self.AMDock.project.bsd_mode_offtarget == 0:
        #     return
        if self.AMDock.project.prog in ['AutoDock4', 'AutoDock4ZN']:
            if self.AMDock.project.mode == 2:
                self.ADout_file = self.AMDock.target.score
            else:
        ...
        # FIXME: print dlg content in log
        if self.AMDock.project.mode == 2:
            return
...
#         # self.AMDock.result_tab.load_button.setEnabled(False)
        ...
        #     print '###### pass'
        self.re = Result_Analysis(self.AMDock.docking_program , self.AMDock.target, os.path.join(
                self.AMDock.project.input, self.AMDock.target.pdbqt), self.AMDock.ligand)
        self.AMDock.output2file.out2file('>  all_poses_target_file: %s\n' % self.AMDock.target.all_poses)
        self.AMDock.output2file.out2file('>  best_pose_target_file: %s\n' % self.AMDock.target.best_pose)
        # else:
        ...
        if self.AMDock.project.mode == 1:
            ...
            # if self.AMDock.docking_program == 'AutoDock Vina':
            self.reB = Result_Analysis(self.AMDock.docking_program, self.AMDock.offtarget, os.path.join(
                self.AMDock.project.input, self.AMDock.offtarget.pdbqt), self.AMDock.ligand)
            self.AMDock.output2file.out2file('>  all_poses_off-target_file: %s\n' % self.AMDock.offtarget.all_poses)
            self.AMDock.output2file.out2file('>  best_pose_off-target_file: %s\n' % self.AMDock.offtarget.best_pose)
            # else:
    ...
    def go_scoring(self):
        ...
        # self.AMDock.result_tab.back.setEnabled(False)
        self.AMDock.result_tab.prot_label.setText('Target: %s' % self.AMDock.target.name)
        self.AMDock.output2file.out2file('>  Target_Protein: %s\n' % self.AMDock.target.name)
    ...
    def scoring(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            ...
            autodock_arg = ['-p', str(self.AMDock.ligand_pdbqt.split('.')[0] + '_' + protein_dpf), '-l',
                            os.path.join(self.AMDock.project.results, self.AMDock.ligand.name +'_' + self.AMDock.target.name + '_score.log')]
            ...
            autodockzn_arg = ['-p', str(self.AMDock.ligand_pdbqt.split('.')[0] + '_' + protein_dpf),
                              '-l', os.path.join(self.AMDock.project.results, self.AMDock.ligand.name +'_' + self.AMDock.target.name + '_score.log')]
    ...
    def process_stoped(self, state, queue):
        ...
        if state:
            ...
            # self.AMDock.log_widget.textedit.append(Ft(/Stopping process...').error())
        else:
            # self.W.stopped = False
            self.timerAD.stop() # stop timer for AutoDock TODO: ver si da error cuando se para cualquier otro prog
    ...
    def start_docking_prog(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            if self.AMDock.project.mode == 2:
                queue.name = 4
                self.AMDock.target.score = os.path.join(self.AMDock.project.results, self.AMDock.ligand.name +'_' + self.AMDock.target.name + '_score.log')
                    ...
                    if self.AMDock.project.bsd_mode_offtarget == 0:
                        for nfill in range(len(self.AMDock.offtarget.fill_list)):
                            ...
                            prepare_dpf_argB = [self.AMDock.prepare_dpf_py, '-l', self.AMDock.ligand.pdbqt, '-r',
                                                ...
                                                '-p', 'rmstol=%s' % self.AMDock.rmsd, '-p',
                                                'ga_num_evals=%s' % self.AMDock.eval,
    ...
    def check_section(self, qname):
        ...
        if self.AMDock.section == 1:
            ...
            self.progressBar_global.setValue(75)
            self.progressBar_section.setValue(0)
            if not self.AMDock.project.bsd_mode_target == 0:
            ...
            if self.AMDock.project.mode == 1 and not self.AMDock.project.bsd_mode_offtarget == 0:
    ...
    def init_prog(self, prog):
        ...
        if prog == 'AutoDock4 B':
            self.AMDock.log_widget.textedit.append(Ft('Determination of better poses for Off-Target Protein...').process())
            self.AMDock.log_widget.textedit.append(Ft('Running AutoDock4...').process())
    ...
    def highlight(self):
        ...
        if self.AMDock.section == -1:
            self.section_state.setText('PROJECT DEFINITION')
        if self.AMDock.section == 0:
            ...
            if self.AMDock.project.mode == 2:
                ...
                self.section_state.setText('SEARCH SPACE DEFINITION')
        elif self.AMDock.section == 2:
