...
class BASE:
    ...
    def get_data(self, filenames):
        if filenames:
            ...
            self.vina_all = 'all_poses' + self.pdbqt_name + '.pdb'
            self.vina_best = 'best_pose' + self.pdbqt_name + '.pdb'
            self.ad4_all = 'all_poses' + self.pdbqt_name + '.pdb'
            self.ad4_best = 'best_pose' + self.pdbqt_name + '.pdb'
...
#         # self.grid_box.setEnabled(False)
        ...
        self.btnA_auto.pressed.connect(lambda: self.grid_prot(self.btnA_auto))
        ...
        self.btnA_res.pressed.connect(lambda: self.grid_prot(self.btnA_res))
        ...
        self.btnA_lig.pressed.connect(lambda: self.grid_prot(self.btnA_lig))
        ...
        self.btnA_user.pressed.connect(lambda: self.grid_prot(self.btnA_user))
        ...
        self.btnB_auto.hide()
        self.btnB_auto.pressed.connect(lambda: self.grid_prot(self.btnB_auto))

        self.btnB_res = QtGui.QRadioButton(self.grid_box)
        self.btnB_res.pressed.connect(lambda: self.grid_prot(self.btnB_res))
        ...
        self.btnB_lig.hide()
        self.btnB_lig.pressed.connect(lambda: self.grid_prot(self.btnB_lig))

        self.btnB_user = QtGui.QRadioButton(self.grid_box)
        self.offtarget_column_group_btnB.addButton(self.btnB_user, 4)
        self.btnB_user.hide()
        self.btnB_user.pressed.connect(lambda: self.grid_prot(self.btnB_user))
    ...
    def prepare_receptor(self):
        ...
        self.size = self.ligand_info.get_gyrate()
        ...
        self.AMDock.log_widget.textedit.append('AMDOCK: IP DOCKING_PROGRAM: %s' %
                                                              self.AMDock.docking_program)
        self.AMDock.log_widget.textedit.append('AMDOCK: IP MODE: %s' % self.AMDock.program_mode)
        self.AMDock.log_widget.textedit.append('AMDOCK: IP TARGET: %s' %
                                                              self.target.name)
        self.AMDock.log_widget.textedit.append('AMDOCK: IP    Hetero: %s' % self.target_info.get_het())
        self.AMDock.log_widget.textedit.append('AMDOCK: IP    Zn atoms: %s' % self.target_info.get_zn())
        if self.AMDock.project.mode == 1:
            self.AMDock.log_widget.textedit.append('AMDOCK: IP OFF-TARGET: %s' % self.offtarget.name)
            self.AMDock.log_widget.textedit.append(
                'AMDOCK: IP    Hetero: %s' % self.offtarget_info.get_het())
            self.AMDock.log_widget.textedit.append('AMDOCK: IP    Zn atoms: %s' %
                                                                  self.offtarget_info.get_zn())
        self.AMDock.log_widget.textedit.append('AMDOCK: IP LIGAND: %s' % self.ligand.name)
        self.AMDock.log_widget.textedit.append('AMDOCK: IP    heavy_atoms: %s' %
    ...
    def binding_site(self):
        ...
        if self.AMDock.project.bsd_mode_target == 0:
            ...
            prepare_gpf4 = {'Prepare_gpf4': [self.AMDock.this_python, [self.AMDock.prepare_gpf4_py, '-l',
                                self.ligand.pdbqt, '-r', self.target.pdbqt, '-p', 'npts={0},{1},{2}'.format(
                    *self.target_info.size), '-p', 'gridcenter={0},{1},{2}'.format(*self.target_info.center), '-p',
                    'spacing=%.3f' % self.AMDock.spacing_autoligand, '-o', self.target.auto_lig]]}
            queue.put(prepare_gpf4)
            autogrid4 = {'AutoGrid4': [self.AMDock.autogrid, ['-p', self.target.auto_lig]]}
            queue.put(autogrid4)
            autoligand = {'AutoLigand': [self.AMDock.this_python, [self.AMDock.autoligand_py, '-r',
                                             self.target.pdbqt_name, '-a', '{}'.format(self.ligand_info.get_ha())]]}
            queue.put(autoligand)

        elif self.AMDock.project.bsd_mode_target == 1:
            self.target.selected = str(self.grid_predef_text.text())
            self.target_info.get_box()
            self.target_info.get_center_selection(self.target.selected)

            prepare_gpf4 = {'Prepare_gpf4': [self.AMDock.this_python, [self.AMDock.prepare_gpf4_py, '-l',
                            self.ligand.pdbqt, '-r', self.target.pdbqt, '-p', 'npts={0},{1},{2}'.format(
                            *self.target_info.size), '-p', 'gridcenter={0},{1},{2}'.format(*self.target_info.center),
                            '-p', 'spacing=%.3f' % self.AMDock.spacing_autoligand, '-o', self.target.auto_lig]]}
            queue.put(prepare_gpf4)
            autogrid4 = {'AutoGrid4': [self.AMDock.autogrid, ['-p', self.target.auto_lig]]}
            queue.put(autogrid4)
            autoligand = {'AutoLigand_point': [self.AMDock.this_python, [self.AMDock.autoligand_py, '-r',
                          self.target.pdbqt_name, '-a', '{}'.format(self.ligand_info.get_ha()), '-x',
            ...
            self.target.selected = str(self.lig_list.currentText())
            self.target_info.get_center_selection(self.target.selected)
            ...
            if self.need_grid:
                ...
                if self.size_x.value() < self.size or self.size_y.value() < self.size or self.size_z.value() < \
                        self.size:
                    self.grid_opt, self.dim_list = smallbox_warning(self, {'x': self.size_x.value(),
                                                                           'y': self.size_y.value(),
                                                                           'z': self.size_z.value()}, self.size,
                                                                    self.target.name)
                    if self.grid_opt == QtGui.QMessageBox.Yes:
                        if 'x' in self.dim_list:
                            self.size_x.setValue(str(self.size))
                        if 'y' in self.dim_list:
                            self.size_y.setValue(str(self.size))
                        if 'z' in self.dim_list:
                            self.size_z.setValue(str(self.size))
        ...
        if self.AMDock.project.mode == 1:
            if self.AMDock.project.bsd_mode_offtarget == 0:
                ...
                prepare_gpf4B = {'Prepare_gpf4 B': [self.AMDock.this_python, [self.AMDock.prepare_gpf4_py, '-l',
                                                                              ...
                                                                              '-o', self.offtarget.auto_lig]]}
                queue.put(prepare_gpf4B)
                autogrid4B = {'AutoGrid4 B': [self.AMDock.autogrid, ['-p', self.offtarget.auto_lig]]}
                queue.put(autogrid4B)
                autoligandB = {'AutoLigand B': [self.AMDock.this_python, [self.AMDock.autoligand_py, '-r',
                                                                          self.offtarget.pdbqt_name, '-a',
                                                                          '{}'.format(self.ligand_info.get_ha())]]}
                queue.put(autoligandB)
            elif self.AMDock.project.bsd_mode_offtarget == 1:
                self.offtarget.selected = str(self.grid_predef_textB.text())
                self.offtarget_info.get_box()
                self.offtarget_info.get_center_selection(self.offtarget.selected)
                prepare_gpf4B = {'Prepare_gpf4 B': [self.AMDock.this_python, [self.AMDock.prepare_gpf4_py, '-l',
                                                                              self.ligand.pdbqt, '-r',
                                                                              self.offtarget.pdbqt, '-p',
                                                                              'npts={0},{1},{2}'.format(
                ...
                autoligandB = {'AutoLigand_point B': [self.AMDock.this_python, [self.AMDock.autoligand_py, '-r',
                                                                          self.offtarget.pdbqt_name, '-a',
                                                                          '{}'.format(self.ligand_info.get_ha()), '-x',
                ...
                self.offtarget.selected = str(self.lig_listB.currentText())
                self.offtarget_info.get_center_selection(self.offtarget.selected)
                ...
                if self.need_gridB:
                    ...
                    obj.close()
                    if self.size_xB.value() < self.size or self.size_yB.value() < self.size or \
                            self.size_zB.value() < self.size:
                        self.grid_optB, self.dim_listB = smallbox_warning(self, {'x': self.size_x.value(),
                                                                                 'y': self.size_y.value(),
                                                                                 'z': self.size_z.value()},
                                                                          self.size, self.offtarget.name)
                        if self.grid_optB == QtGui.QMessageBox.Yes:
                            if 'x' in self.dim_listB:
                                self.size_xB.setValue(self.size)
                            if 'y' in self.dim_listB:
                                self.size_yB.setValue(self.size)
                            if 'z' in self.dim_listB:
                                self.size_zB.setValue(self.size)
        #             self.progressBar.setValue(50)
        self.W.set_queue(queue)  # , 'Prepare Input Files')
        self.W.start_process()
    ...
    def grid_actions(self, btn):
        ...
        if btn.objectName() == 'grid_pymol_button' and btn.text() == 'Show in PyMol':
            visual_arg = [self.AMDock.pymol, self.target.pdbqt, self.AMDock.grid_pymol, '--', '-c',
                          self.AMDock.obj_center, '-s', '%s,%s,%s' % (self.size, self.size, self.size), '-p',
                          'target']
            ...
            if os.path.exists(self.AMDock.obj_center):
                ...
                tf.close()
                self.ttemp = [self.center_x, self.center_y, self.center_z, `self.size`, `self.size`,
                              `self.size`]
            else:
            ...
            visual_arg = [self.AMDock.pymol, self.offtarget.pdbqt, self.AMDock.grid_pymol, '--',
                          '-c',
                          self.AMDock.obj_center1, '-s', '%s,%s,%s' % (self.size, self.size, self.size),
                          '-p', 'off-target']
            ...
            if os.path.exists(self.AMDock.obj_center1):
                ...
                self.ttempB = [self.center_xB, self.center_yB, self.center_zB, `self.size`, `self.size`,
                               `self.size`]
            else:
            ...
            files = []
            dd = '%s*.map' % self.offtarget.name
            files.extend(glob.glob(dd))
            dd = '%s*.fld' % self.offtarget.name
            files.extend(glob.glob(dd))
            dd = '%s*.xyz' % self.offtarget.name
            files.extend(glob.glob(dd))
            dd = '%s*.gpf' % self.offtarget.name
            files.extend(glob.glob(dd))
            ...
            build_arg = [self.AMDock.pymol, self.target.pdbqt, self.AMDock.build_pymol, '--', '-s',
            ...
            build_arg = [self.AMDock.pymol, self.offtarget.pdbqt, self.AMDock.build_pymol, '--',
                         '-s',
                         '%s,%s,%s' % (self.AMDock.rg, self.AMDock.rg,
    ...
    #                 # self.btnB_user.setEnabled(True)
    #                 # self.btnB_lig.setEnabled(True)
    #                 if self.target.pdbqt is not None and self.offtarget.pdbqt is not None and self.AMDock.ligand_pdbqt is not None:
    #                     self.progressBar.setValue(25)
    #             else:
    #                 if self.target.pdbqt is not None and self.AMDock.ligand_pdbqt is not None:
    #                     if self.AMDock.scoring:
    #                         self.progressBar.setValue(50)
    #                     else:
    #                         self.progressBar.setValue(25)
    ...
    def autoligand_out(self, name):
        fill_list = []
        try:
            ofile = open('{}_{}Results.txt'.format(name, self.ligand_info.get_ha() * 6))
        except:
            return
    ...
    def fill_selection(self, autoligand_table):
        if autoligand_table.objectName() == 'autoligand_target':
            ...
            fill_info = PDBINFO('FILL_{}_{}out{}.pdb'.format(self.target.pdbqt_name, self.ligand_info.get_ha()*6, row))
            if fill_info.center:
        ...
        #     fill_info = PDBINFO(
        #         'FILL_{}_{}out{}.pdb'.format(self.offtarget.pdbqt_name, self.ligand_info.get_ha() * 6, row))
        #     fill_info.get_center()
        #     if fill_info.center:
    ...
    def for_finished(self, info):
        ...
        if prog_name == 'AutoLigand':
            ...
            fill_list = self.autoligand_out(self.target.pdbqt_name)
            ...
            # FIXME: check if exitcode
            fill_info = PDBINFO('FILL_{}_{}out1.pdb'.format(self.target.pdbqt_name, self.ligand_info.get_ha() * 6))
        ...
        #             else:
        #                 try:
        #                     os.remove(glob.glob('%s*.map') % self.offtarget.name)
        #                 except:
        #                     pass
        #                 try:
        #                     os.remove(glob.glob('%s*.fld') % self.offtarget.name)
        #                 except:
        #                     pass
        #                 try:
        #                     os.remove(glob.glob('%s*.xyz') % self.offtarget.name)
        #                 except:
        #                     pass
        #                 try:
        #                     os.remove(glob.glob('%s*.gpf') % self.offtarget.name)
        #                 except:
        #                     pass
        #                 ##FIXME
        #                 # self.grid_box.setEnabled(True)
    ...
    def readStdOutput(self):
        ...
        if self.AMDock.project.prog == 'AutoGrid4':
            if re.search('%', self.out):
                # print float(self.out.split()[2].strip('%'))*0.4 - self.AMDock.project.part , '@',

                current = float(self.out.split()[2].strip('%'))*0.4 - self.AMDock.project.part
                # percent = float(self.out.split()[2].strip('%')) * 0.4
                if current < 1:
                    return
    ...
    def autodock_output(self):
        if self.AMDock.project.prog == 'AutoDock4':
            self.ADout_file = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name + '_' + self.target.dlg)
        elif self.AMDock.project.prog == 'AutoDock4ZN':
            self.ADout_file = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name + '_' +
                                           self.target.tzdlg)
        elif self.AMDock.project.prog == 'AutoDock4 B':
            self.ADout_file = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name + '_'
            + self.offtarget.dlg)
        elif self.AMDock.project.prog == 'AutoDock4ZN B':
            self.ADout_file = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name + '_' +
            self.offtarget.tzdlg)
        ...
        for line in ADout:
            ...
            if re.search('DOCKED: MODEL', line):
                ...
                continue
        if self.AMDock.runs_AD <= 40:
            current = float(Ad_count)/self.AMDock.runs_AD * 40 - self.AMDock.project.part
        else:
            current = float(Ad_count)/self.AMDock.runs_AD * 0.4 - self.AMDock.project.part
        if current < 1:
            return
        self.progress(current, self.AMDock.project.mode)
        if self.AMDock.runs_AD <= 40:
            self.AMDock.project.part = float(Ad_count)/self.AMDock.runs_AD * 40
        else:
            self.AMDock.project.part = float(Ad_count)/self.AMDock.runs_AD * 0.4
        ...
        if self.data is not None:
            ...
            if self.AMDock.program_mode is not 'SCORING':
                ...
                if len(errlist) != 0:
                    ...
                    if self.AMDock.project.mode == 1:
                        self.AMDock.result_tab.prot_labelB.setText('Off-Target: %s' % self.offtarget.name)
                        self.AMDock.result_tab.prot_label_selB.setText('%s' % self.offtarget.name)
...
#         # self.AMDock.result_tab.load_button.setEnabled(False)
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            self.re = Result_Analysis(self.target.vina_output, self.target.vina_all,
                                      os.path.join(self.AMDock.project.input, self.target.pdbqt),
                                      self.ligand_info.get_ha())
            self.AMDock.output2file.out2file('>  all_poses_target_file: %s\n' % self.target.vina_all)
            self.AMDock.output2file.out2file('>  best_pose_target_file: %s\n' % self.target.vina_best)
        else:
            self.re = Result_Analysis(self.target.ad4_output, 'all_poses_target_AD4_result.pdb',
                                      os.path.join(self.AMDock.project.input, self.target.pdbqt),
                                      self.ligand_info.get_ha())
            self.AMDock.output2file.out2file('>  all_poses_target_file: %s\n' % self.target.ad4_all)
            self.AMDock.output2file.out2file('>  best_pose_target_file: %s\n' % self.target.ad4_best)
        ...
        if self.AMDock.project.mode == 1:
            ...
            if self.AMDock.docking_program == 'AutoDock Vina':
                vina_resultB = os.path.join(self.AMDock.project.results, self.ligand.name + '_h_out2.pdbqt')
                self.reB = Result_Analysis(vina_resultB, 'all_poses_off-target_ADV_result.pdb',
                                           os.path.join(self.AMDock.project.input, self.offtarget.pdbqt),
                                           self.ligand_info.get_ha())
                ...
                autodock_resultB = os.path.join(self.AMDock.project.results, self.autodock_dlgB)
            ...
            self.AMDock.output2file.out2file(
                ' ______________________________________________________________________________ \n'
                '|                                                                              |\n'
                '|' + (' Result for %s' % self.offtarget.name).ljust(78) + '|\n'
                                                                                         '|______________________________________________________________________________|\n'
                                                                                         '|               |                |              |               |              |\n'
                                                                                         '|     POSES     | BINDING ENERGY | ESTIMATED Ki |    Ki UNITS   |   LIGAND     |\n'
                                                                                         '|               |   (KCAL/MOL)   |              |               |  EFFICIENCY  |\n'
    ...
    def go_scoring(self):
        ...
        self.scoring_re = Scoring2table(self.target.score, ha=self.ligand_info.get_ha())
    ...
    def scoring(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            ...
            protein_gpf = str(self.target.pdbqt.split('.')[0] + '.gpf')
            protein_dlg = str(self.target.pdbqt.split('.')[0] + '.dlg')
            protein_dpf = str(self.target.pdbqt.split('.')[0] + '.dpf')

            prepare_gpf4_arg = [self.AMDock.prepare_gpf4_py, '-l', self.ligand.pdbqt, '-r',
                                self.target.pdbqt, '-f', self.AMDock.gd, '-p', 'spacing=%.3f' %
                                self.AMDock.spacing_autodock, '-p', 'npts=%d,%d,%d' % (
                                    self.size / self.AMDock.spacing_autodock,
                                    self.size / self.AMDock.spacing_autodock,
                                    self.size / self.AMDock.spacing_autodock)]
            ...
            autodock_arg = ['-p', str(self.AMDock.ligand_pdbqt.split('.')[0] + '_' + protein_dpf), '-l',
                            os.path.join(self.AMDock.project.results, self.ligand.name + '_score.log')]
            ...
            protein_TZ = str(self.target.pdbqt.split('.')[0] + '_TZ.pdbqt')
            protein_gpf = str(self.target.pdbqt.split('.')[0] + '_TZ.gpf')
            protein_dlg = str(self.target.pdbqt.split('.')[0] + '_TZ.dlg')
            protein_dpf = str(self.target.pdbqt.split('.')[0] + '_TZ.dpf')
            pseudozn_arg = [self.AMDock.zinc_pseudo_py, '-r', str(self.target.pdbqt)]
            ...
            prepare_gpf4zn_arg = [self.AMDock.prepare_gpf4zn_py, '-l', str(self.AMDock.ligand_pdbqt), '-r',
                                  ...
                                  (self.size / self.AMDock.spacing_autodock,
                                   self.size / self.AMDock.spacing_autodock,
                                   self.size / self.AMDock.spacing_autodock), '-p',
            ...
            autodockzn_arg = ['-p', str(self.AMDock.ligand_pdbqt.split('.')[0] + '_' + protein_dpf),
                              '-l', os.path.join(self.AMDock.project.results, self.ligand.name + '_score.log')]
    ...
    def start_docking_prog(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            if self.AMDock.project.mode == 2:
                queue.name = 4
                self.target.score = os.path.join(self.AMDock.project.results, self.ligand.name + '_score.log')
                vina_score = {'AutoDock Vina Scoring': [self.AMDock.vina_exec, ['--receptor', self.target.pdbqt,
                                                                                '--ligand', self.ligand.pdbqt,
                                                                                '--score_only', "--log",
                                                                                self.target.score]]}
                queue.put(vina_score)
            else:
                queue.name = 3
                self.target.vina_output = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name +'_' +
                                                        self.target.name + '_out.pdbqt')
                self.target.vina_log = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name + '_' +
                                                        self.target.name + '_out.log')
                vina_arg = ['--receptor', self.target.pdbqt, '--ligand', self.ligand.pdbqt, '--center_x',
                            '{}'.format(self.grid_center[0]), '--center_y', '{}'.format(self.grid_center[1]),
                            '--center_z', '{}'.format(self.grid_center[2]), '--size_x', '{}'.format(
                            self.size),'--size_y','{}'.format(self.size), '--size_z', '{}'.format(
                            self.size), '--cpu', str(self.AMDock.ncpu), '--num_modes', str(self.AMDock.poses_vina),
                            '--exhaustiveness', str(self.AMDock.exhaustiveness), '--out', self.target.vina_output ,
                            "--log", self.target.vina_log]
                vina = {'AutoDock Vina': [self.AMDock.vina_exec, vina_arg]}
                queue.put(vina)
                if self.AMDock.project.mode == 1:

                    vina_argB = ['--receptor', self.offtarget.pdbqt, '--ligand', self.ligand.pdbqt,'--center_x',
                                '{}'.format(self.grid_centerB[0]), '--center_y', '{}'.format(self.grid_centerB[1]),
                                '--center_z', '{}'.format(self.grid_centerB[2]), '--size_x', '{}'.format(
                                self.size),'--size_y','{}'.format(self.size), '--size_z', '{}'.format(
                                self.size), '--cpu', str(self.AMDock.ncpu), '--num_modes', str(self.AMDock.poses_vina),
                                '--exhaustiveness', str(self.AMDock.exhaustiveness), '--out', os.path.join(
                                self.AMDock.project.results, self.ligand.pdbqt_name + self.target.name + '_out2.pdbqt'),
                                 "--log", os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name + '_' +
                                                       self.offtarget.name + '_out.log')]
            ...
            if self.AMDock.project.mode == 2:
                ...
                self.target.score = os.path.join(self.AMDock.project.results, self.ligand.name + '_score.log')
                prepare_gpf4_arg = [self.AMDock.prepare_gpf4_py, '-l', self.ligand.pdbqt, '-r',
                                    self.target.pdbqt, '-p', 'spacing=%.3f' % self.AMDock.spacing_autodock, '-p',
                                    'npts=%d,%d,%d' % (self.size / self.AMDock.spacing_autodock,
                                    self.size / self.AMDock.spacing_autodock, self.size / self.AMDock.spacing_autodock),
                                    '-p', 'gridcenter={0},{1},{2}'.format(*self.ligand_info.center)]
                prepare_gpf4 = {'Prepare_gpf4': [self.AMDock.this_python, prepare_gpf4_arg]}
                queue.put(prepare_gpf4)
                autogrid4 = {'AutoGrid4': [self.AMDock.autogrid, ['-p', self.target.gpf]]}
                queue.put(autogrid4)
                prepare_dpf_arg = [self.AMDock.prepare_dpf_py, '-l', self.ligand.pdbqt, '-r',
                                   self.target.pdbqt, '-e']
                prepare_dpf4 = {'Prepare_dpf4': [self.AMDock.this_python, prepare_dpf_arg]}
                queue.put(prepare_dpf4)
                autodock_arg = ['-p', self.ligand.pdbqt_name + '_' + self.target.dpf, '-l', self.target.score]
                autodock = {'AutoDock4': [self.AMDock.autodock, autodock_arg]}
                queue.put(autodock)
            else:
                queue.name = 3

                self.target.ad4_output = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name + '_' +
                                                      self.target.dlg)
                prepare_gpf4_arg = [self.AMDock.prepare_gpf4_py, '-l', self.ligand.pdbqt, '-r',
                                    self.target.pdbqt, '-p', 'spacing=%.3f' % self.AMDock.spacing_autodock, '-p',
                                    'npts=%d,%d,%d' % (self.size / self.AMDock.spacing_autodock, self.size /
                                                       self.AMDock.spacing_autodock, self.size /
                ...
                queue.put(autogrid4)
                prepare_dpf_arg = [self.AMDock.prepare_dpf_py, '-l', self.ligand.pdbqt, '-r',
                                   self.target.pdbqt, '-p', 'rmstol=%s' % self.AMDock.rmsd, '-p',
                                   'ga_num_evals=%s' % self.AMDock.eval, '-p', 'ga_run=%s' % self.AMDock.runs]
                prepare_dpf4 = {'Prepare_dpf4': [self.AMDock.this_python, prepare_dpf_arg]}
                queue.put(prepare_dpf4)
                autodock_arg = ['-p', self.ligand.pdbqt_name + '_' + self.target.dpf, '-l', self.target.ad4_output]
                autodock = {'AutoDock4': [self.AMDock.autodock, autodock_arg]}
                queue.put(autodock)

                if self.AMDock.project.mode == 1:
                    prepare_gpf4_argB = [self.AMDock.prepare_gpf4_py, '-l', self.ligand.pdbqt, '-r',
                                    self.offtarget.pdbqt, '-p', 'spacing=%.3f' % self.AMDock.spacing_autodock, '-p',
                                    'npts=%d,%d,%d' % (self.size / self.AMDock.spacing_autodock, self.size /
                                                       self.AMDock.spacing_autodock, self.size /
                                                       self.AMDock.spacing_autodock), '-p', 'gridcenter={0},{1},'
                    ...
                    autogrid4B = {'AutoGrid4 B': [self.AMDock.autogrid, ['-p', self.offtarget.gpf]]}
                    queue.put(autogrid4B)

                    prepare_dpf_argB = [self.AMDock.prepare_dpf_py, '-l', self.ligand.pdbqt, '-r', self.offtarget.pdbqt,
                    ...
                    autodock_argB = ['-p', self.ligand.pdbqt_name + '_' + self.offtarget.dpf, '-l', ]
            ...
            if self.AMDock.project.mode == 2:
                pseudozn = { 'PseudoZn': [self.AMDock.this_python, [self.AMDock.zinc_pseudo_py, '-r',
                                                                    self.target.pdbqt]]}
                queue.put(pseudozn)
                prepare_gpf4zn_arg = [self.AMDock.prepare_gpf4zn_py, '-l', str(self.ligand.pdbqt), '-r',
                                      self.target.tzpdbqt, '-p', 'spacing=%.3f' % self.AMDock.spacing_autodock, '-p',
                                      'npts=%d,%d,%d' % (self.size / self.AMDock.spacing_autodock, self.size /
                                      self.AMDock.spacing_autodock, self.size / self.AMDock.spacing_autodock), '-p',
                ...
                autogrid4 = {'AutoGrid4': [self.AMDock.autogrid, ['-p', self.target.tzgpf]]}
                queue.put(autogrid4)
                prepare_dfp4zn = {'Prepare_dpf4': [self.AMDock.this_python, [self.AMDock.prepare_dpf_py, '-l',
                                                  self.ligand.pdbqt, '-r', self.target.tzpdbqt, '-e']]}
                queue.put(prepare_dfp4zn)
                autodockzn_arg = ['-p', self.ligand.pdbqt_name + '_' + self.target.tzdpf, '-l', self.target.score]
                autodockzn = {'AutoDock4ZN': [self.AMDock.autodock, autodockzn_arg]}
                queue.put(autodockzn)
            else:

                pseudozn = {'PseudoZn': [self.AMDock.this_python, [self.AMDock.zinc_pseudo_py, '-r', self.target.pdbqt]]}
                queue.put(pseudozn)
                prepare_gpf4zn_arg = [self.AMDock.prepare_gpf4zn_py, '-l', str(self.ligand.pdbqt), '-r',
                                      self.target.tzpdbqt,'-p', 'spacing=%.3f' % self.AMDock.spacing_autodock, '-p',
                                    'npts=%d,%d,%d' % (self.size/ self.AMDock.spacing_autodock, self.size/
                ...
                prepare_dfp4zn = {'Prepare_dpf4': [self.AMDock.this_python, [self.AMDock.prepare_dpf_py, '-l',
                                       self.ligand.pdbqt, '-r', self.target.tzpdbqt, '-p', 'rmstol=%s' % self.AMDock.rmsd,
                                     '-p', 'ga_num_evals=%s' % self.AMDock.eval, '-p', 'ga_run=%s' % self.AMDock.runs]]}
                queue.put(prepare_dfp4zn)

                autodockzn_arg = ['-p', self.ligand.pdbqt_name + '_' + self.target.tzdpf, '-l', os.path.join(
                    self.AMDock.project.results, self.ligand.pdbqt_name + '_' + self.target.tzdlg)]
                autodockzn = {'AutoDock4ZN': [self.AMDock.autodock, autodockzn_arg]}
                queue.put(autodockzn)

                if self.AMDock.project.mode == 1:
                    pseudoznB = {'PseudoZn B': [self.AMDock.this_python, [self.AMDock.zinc_pseudo_py, '-r',
                                                                        self.offtarget.pdbqt]]}
                    queue.put(pseudoznB)

                    prepare_gpf4zn_argB = [self.AMDock.prepare_gpf4zn_py, '-l', self.ligand.pdbqt, '-r',
                                          self.offtarget.tzpdbqt, '-p', 'spacing=%.3f' % self.AMDock.spacing_autodock, '-p',
                                          'npts=%d,%d,%d' % (self.size / self.AMDock.spacing_autodock, self.size /
                                          self.AMDock.spacing_autodock, self.size / self.AMDock.spacing_autodock),
                    ...
                    autogrid4B = {'AutoGrid4 B': [self.AMDock.autogrid, ['-p', self.offtarget.tzgpf]]}
                    queue.put(autogrid4B)

                    prepare_dfp4znB = {'Prepare_dpf4 B': [self.AMDock.this_python, [self.AMDock.prepare_dpf_py, '-l',
                                        self.ligand.pdbqt, '-r', self.offtarget.tzpdbqt, '-p', 'rmstol=%s' %
                                        self.AMDock.rmsd, '-p', 'ga_num_evals=%s' % self.AMDock.eval, '-p', 'ga_run=%s' %
                                        self.AMDock.runs]]}
                    queue.put(prepare_dfp4znB)

                    autodockzn_argB = ['-p', self.ligand.pdbqt_name + '_' + self.offtarget.tzdpf, '-l', os.path.join(
                        self.AMDock.project.results, self.ligand.pdbqt_name + '_' + self.offtarget.tzdlg)]
    ...
    def check_section(self, qname):
        ...
        if self.AMDock.section == 1:
            ...
            self.size_x.setValue(self.size)
            self.size_y.setValue(self.size)
            self.size_z.setValue(self.size)
            self.AMDock.log_widget.textedit.append('AMDOCK: IF > Prepare Initial Files...Done.\n')
        elif self.AMDock.section == 2:
