...
class BASE:
    ...
    def get_data(self, filenames):
        if filenames:
            ...
            self.vina_all = 'all_poses' + '_' + self.pdbqt_name + '.pdb'
            self.vina_best = 'best_pose' + '_' + self.pdbqt_name + '.pdb'
            self.ad4_all = 'all_poses' + '_' + self.pdbqt_name + '.pdb'
            self.ad4_best = 'best_pose' + '_' + self.pdbqt_name + '.pdb'
...
#         # self.grid_box.setEnabled(False)
    ...
    def prepare_receptor(self):
        ...
        self.size = math.ceil(self.ligand_info.get_gyrate())
    ...
    def grid_actions(self, btn):
        ...
        if btn.objectName() == 'grid_pymol_button':
            visual_arg = [self.AMDock.pymol, self.target.pdbqt, self.AMDock.grid_pymol,
                          ...
                          '-s', '{},{},{}'.format(self.size, self.size, self.size), '-p', 'target']
            ...
            visual_arg = [self.AMDock.pymol, self.offtarget.pdbqt, self.AMDock.grid_pymol, '--',
                          ...
                          '-s', '{},{},{}'.format(self.size, self.size, self.size), '-p', 'off-target']
...
#         # self.AMDock.result_tab.load_button.setEnabled(False)
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            self.re = Result_Analysis(self.target.vina_output, self.target.vina_all, self.target.vina_best,
            ...
            self.re = Result_Analysis(self.target.ad4_output, self.target.ad4_all, self.target.ad4_best,
                                      os.path.join(self.AMDock.project.input, self.target.pdbqt),
                                      self.ligand_info.get_ha())
            self.AMDock.output2file.out2file('>  all_poses_target_file: %s\n' % self.target.ad4_all)
        ...
        if self.AMDock.project.mode == 1:
            ...
            if self.AMDock.docking_program == 'AutoDock Vina':
                self.reB = Result_Analysis(self.offtarget.vina_output, self.offtarget.vina_all,
                                           self.offtarget.vina_best,
                                           os.path.join(self.AMDock.project.input, self.offtarget.pdbqt),
                                           self.ligand_info.get_ha())
                self.AMDock.output2file.out2file('>  all_poses_off-target_file: %s\n' % self.offtarget.vina_all)
                self.AMDock.output2file.out2file('>  best_pose_off-target_file: %s\n' % self.offtarget.vina_best)
            else:
                self.reB = Result_Analysis(self.offtarget.ad4_output, self.offtarget.ad4_all, self.offtarget.ad4_best,
                                           os.path.join(self.AMDock.project.input, self.offtarget.pdbqt),
                                           self.ligand_info.get_ha())
                self.AMDock.output2file.out2file('>  all_poses_off-target_file: %s\n' % self.offtarget.ad4_all)
                self.AMDock.output2file.out2file('>  best_pose_off-target_file: %s\n' % self.offtarget.ad4_best)
    ...
    def start_docking_prog(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            if self.AMDock.project.mode == 2:
                ...
                queue.put(vina)
                if self.AMDock.project.mode == 1:
                    self.offtarget.vina_output = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name +
                                                              '_' + self.offtarget.name + '_out.pdbqt')
                    self.offtarget.vina_log = os.path.join(self.AMDock.project.results, self.ligand.pdbqt_name + '_' +
                    ...
                    vina_argB = ['--receptor', self.offtarget.pdbqt, '--ligand', self.ligand.pdbqt,'--center_x',
                                ...
                                '--exhaustiveness', str(self.AMDock.exhaustiveness), '--out',
                                 self.offtarget.vina_output, "--log", self.offtarget.vina_log]
