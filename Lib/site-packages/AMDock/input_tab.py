...
#         # self.AMDock.result_tab.load_button.setEnabled(False)
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            ...
            autodock_result = os.path.join(self.AMDock.project.results, self.target.dlg)
    ...
    def go_scoring(self):
        ...
        scoring_file = os.path.join(self.AMDock.project.results, self.ligand.name + '_score.log')
        self.scoring_re = Scoring2table(scoring_file, ha=self.ligand_info.get_ha())
    ...
    def scoring(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            vina_score = {'AutoDock Vina Scoring': [self.AMDock.vina_exec, ['--receptor', self.target.pdbqt,
                                '--ligand', self.ligand.pdbqt, '--score_only', "--log", os.path.join(
                                self.AMDock.project.results, self.ligand.name + '_score.log')]]}
    ...
    def start_docking_prog(self):
        ...
        if self.AMDock.docking_program == 'AutoDock Vina':
            ...
            autodock_arg = ['-p', self.ligand.pdbqt_name + '_' + self.target.dpf, '-l', os.path.join(
                self.AMDock.project.results, self.ligand.pdbqt_name + '_' + self.target.dlg)]
            ...
            if self.AMDock.project.mode == 1:
                ...
                autodock_argB = ['-p', self.ligand.pdbqt_name + '_' + self.offtarget.dpf, '-l', os.path.join(
                    self.AMDock.project.results, self.ligand.pdbqt_name + '_' + self.offtarget.dlg)]
