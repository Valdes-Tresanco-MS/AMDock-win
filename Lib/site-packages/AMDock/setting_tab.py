...
class Configuration_tab(QtGui.QWidget):
    ...
    def _align(self):
        self.AMDock.prot_align = self.align.isChecked()
        ...
        for line in cfile:
            ...
            if re.search('#', line) or re.search('\[', line):
                ...
                if re.search('cpu', line):
                    ...
                    if line.split()[1] == 'False':
                        self.prot_align = self.AMDock.prot_align = False
                    else:
                        self.prot_align = self.AMDock.prot_align = True
                try:
                    self.align.setChecked(self.AMDock.prot_align)
                except:
                    pass
    ...
    def check_changes(self):
        ...
        if (self.ncpu != self.AMDock.ncpu or self.exhaustiveness != self.AMDock.exhaustiveness or
                ...
                self.prot_align != self.AMDock.prot_align or self.log != self.AMDock.log or
            self.log_level != self.AMDock.log_level):
