...
class Configuration_tab(QtGui.QWidget):
    ...
    def conf_from_file(self):
        ...
        self.exh_value.setProperty('value', self.AMDock.exhaustiveness)
        self.nposes_value.setProperty('value', self.AMDock.poses_vina)
        self.neval_value.setProperty('value', self.AMDock.eval)
        self.nruns_value.setProperty('value', self.AMDock.runs)
        self.rmstol_value.setProperty('value', self.AMDock.rmsd)
        self.horizontalSlider.setProperty('value', self.AMDock.ncpu)
        self.log_view.setChecked(self.AMDock.log)
        self.log_group.button(self.AMDock.log_level).setChecked(True)
        self.align.setChecked(self.AMDock.prot_align)
    ...
    def configuration(self):
        ...
        self.AMDock.statusbar.showMessage('Wrote configuration file', 5000)
        ...
        for line in cfile:
            ...
            if re.search('#', line) or re.search('\[', line):
                ...
                if re.search('ff', line):
                    ...
                    self.AMDock.ncpu = int(line.split()[1])
                elif re.search('exhaustiveness', line):
                    self.AMDock.exhaustiveness = int(line.split()[1])
                elif re.search('NoPoses', line):
                    self.AMDock.poses_vina = int(line.split()[1])
                elif re.search('ga_num_eval', line):
                    self.AMDock.eval = int(line.split()[1])
                elif re.search('ga_run', line):
                    self.AMDock.runs = int(line.split()[1])
                elif re.search('rmstol', line):
                    self.AMDock.rmsd = float(line.split()[1])
                elif re.search('log', line):
                    if line.split()[1] == 'False':
                        self.AMDock.log = False
                    else:
                        self.AMDock.log = True
                elif re.search('log_level', line):
                    self.AMDock.log_level = int(line.split()[1])
                elif re.search('prot_align', line):
                    if line.split()[1] == 'False':
                        self.AMDock.prot_align = False
                    else:
                        self.AMDock.prot_align = True
                try:
                    self.align.setChecked(self.AMDock.prot_align)
                except:
                    pass
    ...
    def check_changes(self):
        ...
        if self.cpu != self.AMDock.ncpu or self.exhaustiveness != self.AMDock.exhaustiveness or self.NoPoses != \
                self.AMDock.poses_vina or self.forcefield != self.AMDock.forcefield or self.ga_run != self.AMDock.runs \
                or self.ga_num_eval != self.AMDock.eval or self.rmstol != self.AMDock.rmsd or self.alignment != self.AMDock.prot_align:
