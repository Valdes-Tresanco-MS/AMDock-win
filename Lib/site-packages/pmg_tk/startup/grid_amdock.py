...
class AMDockPlugin:
    ...
    def read_data(self):
        if os.path.exists('pymol_data.txt'):
            ...
            for line in file:
                ...
                elif line.startswith('offtarget'):
    ...
    def save_info(self):
        savet = True
        saveo = True
        if [x.get() for x in self.target_grid_size]== [int(x) for x in self.target_data[1:4]] and \
                [x.get() for x in self.target_grid_center]== [float(x) for x in self.target_data[4:]]:
            savet = False
        if [x.get() for x in self.offtarget_grid_size]== [int(x) for x in self.offtarget_data[1:4]] and \
                [x.get() for x in self.offtarget_grid_center]== [float(x) for x in self.offtarget_data[4:]]:
            saveo = False
    ...
    def check_for_exit(self, btn):
        if btn == 'Save Box Info':
            ...
            if [x.get() for x in self.target_grid_size] != [int(x) for x in self.target_data[1:4]] or \
                [x.get() for x in self.target_grid_center] != [float(x) for x in self.target_data[4:]] or \
                [x.get() for x in self.offtarget_grid_size] != [int(x) for x in self.offtarget_data[1:4]] or \
                [x.get() for x in self.offtarget_grid_center] != [float(x) for x in self.offtarget_data[4:]]:
