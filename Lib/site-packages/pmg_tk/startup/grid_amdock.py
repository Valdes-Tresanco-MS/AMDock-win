        ...
        if result == 'Save Box Information':
            if self.protein == 'target':
                ...
                if os.path.exists('user_target_dim.txt'):
                    ok = tkMessageBox.showinfo('Information', 'Dimension data was saved correctly.')
                else:
                    error = tkMessageBox.showerror('Error', 'An error occurred while saving the information')

            elif self.protein == 'off-target':
                control_file = open('user_off_target_dim.txt', 'w')
                control_file.write('%s %s %s %s %s %s' % (self.grid_center[0].get(), self.grid_center[1].get(),
                                                         self.grid_center[2].get(), self.size_x.get(),
                                                         self.size_y.get(), self.size_z.get()))
                control_file.close()
                if os.path.exists('user_off_target_dim.txt'):
                    ok = tkMessageBox.showinfo('info', 'Dimension data was saved correctly.')
                else:
                    error = tkMessageBox.showerror('Error', 'An error occurred while saving the information')
        elif result == None:
            exit = tkMessageBox.askokcancel('Warning', 'Do really you wish to quit?234234')
            if exit:
